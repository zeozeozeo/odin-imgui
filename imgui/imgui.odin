...
TreeNodeFlags_AllowOverlap :: TreeNodeFlags{.AllowOverlap}
...
TreeNodeFlags_AllowItemOverlap :: TreeNodeFlags{.AllowOverlap}
...
SelectableFlags_AllowOverlap :: SelectableFlags{.AllowOverlap}
SelectableFlags_AllowItemOverlap :: SelectableFlags{.AllowOverlap}
...
/* this value is quite unfortunate */// 	InvalidMask_ = 30, // [Internal] We treat using those bits as being potentially a 'float power' argument from the previous API that has got miscast to this enum, and will trigger an assert if needed.
}
...
Style :: struct {
	...
	AntiAliasedLines: c.bool, // Enable anti-aliased lines/borders. Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).
	AntiAliasedLinesUseTex: c.bool, // Enable anti-aliased lines/borders using textures where possible. Require backend to render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList).
	AntiAliasedFill: c.bool, // Enable anti-aliased edges around filled shapes (rounded rectangles, circles, etc.). Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).
...
KeyData :: struct {
	Down: c.bool, // True for if key is down
...
IO :: struct {
	...
	FontAllowUserScaling: c.bool, // = false          // Allow user scaling text of individual window with CTRL+Wheel.
	...
	ConfigDockingNoSplit: c.bool, // = false          // Simplified docking mode: disable window splitting, so docking is limited to merging multiple windows together into tab-bars.
	ConfigDockingWithShift: c.bool, // = false          // Enable docking with holding Shift key (reduce visual noise, allows dropping in wider space)
	ConfigDockingAlwaysTabBar: c.bool, // = false          // [BETA] [FIXME: This currently creates regression with auto-sizing and general overhead] Make every single floating window display within a docking node.
	ConfigDockingTransparentPayload: c.bool, // = false          // [BETA] Make window or viewport transparent when docking and only display docking boxes on the target viewport. Useful if rendering of multiple viewport cannot be synced. Best used with ConfigViewportsNoAutoMerge.
	// Viewport options (when ImGuiConfigFlags_ViewportsEnable is set)
	ConfigViewportsNoAutoMerge: c.bool, // = false;         // Set to make all floating imgui windows always create their own viewport. Otherwise, they are merged into the main host viewports when overlapping it. May also set ImGuiViewportFlags_NoAutoMerge on individual viewport.
	ConfigViewportsNoTaskBarIcon: c.bool, // = false          // Disable default OS task bar icon flag for secondary viewports. When a viewport doesn't want a task bar icon, ImGuiViewportFlags_NoTaskBarIcon will be set on it.
	ConfigViewportsNoDecoration: c.bool, // = true           // Disable default OS window decoration flag for secondary viewports. When a viewport doesn't want window decorations, ImGuiViewportFlags_NoDecoration will be set on it. Enabling decoration can create subsequent issues at OS levels (e.g. minimum window size).
	ConfigViewportsNoDefaultParent: c.bool, // = false          // Disable default OS parenting to main viewport for secondary viewports. By default, viewports are marked with ParentViewportId = <main_viewport>, expecting the platform backend to setup a parent/child relationship between the OS windows (some backend may ignore this). Set to true if you want the default to be 0, then all viewports will be top-level OS windows.
	// Miscellaneous options
	MouseDrawCursor: c.bool, // = false          // Request ImGui to draw a mouse cursor for you (if you are on a platform without a mouse cursor). Cannot be easily renamed to 'io.ConfigXXX' because this is frequently used by backend implementations.
	ConfigMacOSXBehaviors: c.bool, // = defined(__APPLE__) // OS X style: Text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start and End using Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl.
	ConfigInputTrickleEventQueue: c.bool, // = true           // Enable input queue trickling: some types of events submitted during the same frame (e.g. button down + up) will be spread over multiple frames, improving interactions with low framerates.
	ConfigInputTextCursorBlink: c.bool, // = true           // Enable blinking cursor (optional as some users consider it to be distracting).
	ConfigInputTextEnterKeepActive: c.bool, // = false          // [BETA] Pressing Enter will keep item active and select contents (single-line only).
	ConfigDragClickToInputText: c.bool, // = false          // [BETA] Enable turning DragXXX widgets into text input with a simple mouse click-release (without moving). Not desirable on devices without a keyboard.
	ConfigWindowsResizeFromEdges: c.bool, // = true           // Enable resizing of windows from their edges and from the lower-left corner. This requires (io.BackendFlags & ImGuiBackendFlags_HasMouseCursors) because it needs mouse cursor feedback. (This used to be a per-window ImGuiWindowFlags_ResizeFromAnySide flag)
	ConfigWindowsMoveFromTitleBarOnly: c.bool, // = false       // Enable allowing to move windows only when clicking on their title bar. Does not apply to windows without a title bar.
	...
	ConfigDebugBeginReturnValueOnce: c.bool, // = false          // First-time calls to Begin()/BeginChild() will return false. NEEDS TO BE SET AT APPLICATION BOOT TIME if you don't want to miss windows.
	ConfigDebugBeginReturnValueLoop: c.bool, // = false          // Some calls to Begin()/BeginChild() will return false. Will cycle through window depths then repeat. Suggested use: add "io.ConfigDebugBeginReturnValue = io.KeyShift" in your main loop then occasionally press SHIFT. Windows should be flickering while running.
	...
	ConfigDebugIgnoreFocusLoss: c.bool, // = false          // Ignore io.AddFocusEvent(false), consequently not calling io.ClearInputKeys() in input processing.
	// Option to audit .ini data
	ConfigDebugIniSettings: c.bool, // = false          // Save .ini data with extra comments (particularly helpful for Docking, but makes saving slower)
	...
	WantCaptureMouse: c.bool, // Set when Dear ImGui will use mouse inputs, in this case do not dispatch them to your main game/application (either way, always pass on mouse inputs to imgui). (e.g. unclicked mouse is hovering over an imgui window, widget is active, mouse was clicked over an imgui window, etc.).
	WantCaptureKeyboard: c.bool, // Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to your main game/application (either way, always pass keyboard inputs to imgui). (e.g. InputText active, or an imgui window is focused and navigation is enabled, etc.).
	WantTextInput: c.bool, // Mobile/console: when set, you may display an on-screen keyboard. This is set by Dear ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).
	WantSetMousePos: c.bool, // MousePos has been altered, backend should reposition mouse on next frame. Rarely used! Set only when ImGuiConfigFlags_NavEnableSetMousePos flag is enabled.
	WantSaveIniSettings: c.bool, // When manual .ini load/save is active (io.IniFilename == NULL), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. Important: clear io.WantSaveIniSettings yourself after saving!
	NavActive: c.bool, // Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX events) = a window is focused and it doesn't use the ImGuiWindowFlags_NoNavInputs flag.
	NavVisible: c.bool, // Keyboard/Gamepad navigation is visible and allowed (will handle ImGuiKey_NavXXX events).
	...
	MouseDown: [5]c.bool, // Mouse buttons: 0=left, 1=right, 2=middle + extras (ImGuiMouseButton_COUNT == 5). Dear ImGui mostly uses left and right buttons. Other buttons allow us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** API.
	...
	KeyCtrl: c.bool, // Keyboard modifier down: Control
	KeyShift: c.bool, // Keyboard modifier down: Shift
	KeyAlt: c.bool, // Keyboard modifier down: Alt
	KeySuper: c.bool, // Keyboard modifier down: Cmd/Super/Windows
	...
	WantCaptureMouseUnlessPopupClose: c.bool, // Alternative to WantCaptureMouse: (WantCaptureMouse == true && WantCaptureMouseUnlessPopupClose == false) when a click over void is expected to close a popup.
	...
	MouseWheelRequestAxisSwap: c.bool, // On a non-Mac system, holding SHIFT requests WheelY to perform the equivalent of a WheelX event. On a Mac system this is already enforced by the system.
...
InputTextCallbackData :: struct {
	...
	BufDirty: c.bool, // Set if you modify Buf/BufTextLen!    // Write        // [Completion,History,Always]
...
WindowClass :: struct {
	...
	DockingAlwaysTabBar: c.bool, // Set to true to enforce single floating windows of this class always having their own docking node (equivalent of setting the global io.ConfigDockingAlwaysTabBar)
	DockingAllowUnclassed: c.bool, // Set to true to allow windows of this class to be docked/merged with an unclassed window. // FIXME-DOCK: Move to DockNodeFlags override?
}
...
Payload :: struct {
	...
	Preview: c.bool, // Set when AcceptDragDropPayload() was called and mouse has been hovering the target item (nb: handle overlapping drag targets)
	Delivery: c.bool, // Set when AcceptDragDropPayload() was called and mouse button is released over the target item.
}
...
TableSortSpecs :: struct {
	...
	SpecsDirty: c.bool, // Set to true when specs have changed since last time! Use this to sort again, then clear the flag.
}
...
DrawData :: struct {
	Valid: c.bool, // Only valid after Render() is called and before the next NewFrame() is called.
...
FontConfig :: struct {
	...
	FontDataOwnedByAtlas: c.bool, // true     // TTF/OTF data ownership taken by the container ImFontAtlas (will delete memory itself).
	...
	PixelSnapH: c.bool, // false    // Align every glyph to pixel boundary. Useful e.g. if you are merging a non-pixel aligned font with the default font. If enabled, you can set OversampleH/V to 1.
	...
	MergeMode: c.bool, // false    // Merge into previous ImFont, so you can combine multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to use GlyphOffset.y when merge font of different heights.
...
FontAtlas :: struct {
	...
	Locked: c.bool, // Marked as Locked by ImGui::NewFrame() so attempt to modify the atlas will assert.
	UserData: rawptr, // Store your own atlas related user-data (if e.g. you have multiple font atlas).
	// [Internal]
	// NB: Access texture data via GetTexData*() calls! Which will setup a default font for you.
	TexReady: c.bool, // Set when texture was built matching current font input
	TexPixelsUseColors: c.bool, // Tell whether our texture data is known to use colors (rather than just alpha channel), in order to help backend select a format.
...
Font :: struct {
	...
	DirtyLookupTables: c.bool, // 1     // out //
...
Viewport :: struct {
	...
	PlatformWindowCreated: c.bool, // Platform window has been created (Platform_CreateWindow() has been called). This is false during the first frame where a viewport is being created.
	PlatformRequestMove: c.bool, // Platform window requested move (e.g. window was moved by the OS / host window manager, authoritative position will be OS window position)
	PlatformRequestResize: c.bool, // Platform window requested resize (e.g. window was resized by the OS / host window manager, authoritative size will be OS window size)
	PlatformRequestClose: c.bool, // Platform window requested closure (e.g. window was moved by the OS / host window manager, e.g. pressing ALT-F4)
}
...
PlatformIO :: struct {
	...
	Platform_GetWindowFocus: proc "c" (vp: ^Viewport) -> c.bool, // . . U . .  //
	Platform_GetWindowMinimized: proc "c" (vp: ^Viewport) -> c.bool, // N . . . .  // Get platform window minimized state. When minimized, we generally won't attempt to get/set size and contents will be culled more easily
...
PlatformImeData :: struct {
	WantVisible: c.bool, // A widget wants the IME to be visible
...
foreign lib {
	...
	@(link_name="ImGui_DestroyContext") DestroyContext :: proc "c" (ctx: ^Context) --- // NULL = destroy current context
	...
	// Main
	@(link_name="ImGui_GetIO") GetIO :: proc "c" () -> ^IO --- // access the IO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)
	@(link_name="ImGui_GetStyle") GetStyle :: proc "c" () -> ^Style --- // access the Style structure (colors, sizes). Always use PushStyleCol(), PushStyleVar() to modify style mid-frame!
	@(link_name="ImGui_NewFrame") NewFrame :: proc "c" () --- // start a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().
	@(link_name="ImGui_EndFrame") EndFrame :: proc "c" () --- // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
	@(link_name="ImGui_Render") Render :: proc "c" () --- // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
	@(link_name="ImGui_GetDrawData") GetDrawData :: proc "c" () -> ^DrawData --- // valid after Render() and until the next call to NewFrame(). this is what you have to render.
	// Demo, Debug, Information
	@(link_name="ImGui_ShowDemoWindow") ShowDemoWindow :: proc "c" (p_open: ^c.bool) --- // create Demo window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
	@(link_name="ImGui_ShowMetricsWindow") ShowMetricsWindow :: proc "c" (p_open: ^c.bool) --- // create Metrics/Debugger window. display Dear ImGui internals: windows, draw commands, various internal state, etc.
	@(link_name="ImGui_ShowDebugLogWindow") ShowDebugLogWindow :: proc "c" (p_open: ^c.bool) --- // create Debug Log window. display a simplified log of important dear imgui events.
	@(link_name="ImGui_ShowStackToolWindow") ShowStackToolWindow :: proc "c" (p_open: ^c.bool) --- // create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
	@(link_name="ImGui_ShowAboutWindow") ShowAboutWindow :: proc "c" (p_open: ^c.bool) --- // create About window. display Dear ImGui version, credits and build/system information.
	@(link_name="ImGui_ShowStyleEditor") ShowStyleEditor :: proc "c" (ref: ^Style) --- // add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
	@(link_name="ImGui_ShowStyleSelector") ShowStyleSelector :: proc "c" (label: cstring) -> c.bool --- // add style selector block (not a window), essentially a combo listing the default styles.
	@(link_name="ImGui_ShowFontSelector") ShowFontSelector :: proc "c" (label: cstring) --- // add font selector block (not a window), essentially a combo listing the loaded fonts.
	@(link_name="ImGui_ShowUserGuide") ShowUserGuide :: proc "c" () --- // add basic help/info block (not a window): how to manipulate ImGui as an end-user (mouse/keyboard controls).
	@(link_name="ImGui_GetVersion") GetVersion :: proc "c" () -> cstring --- // get the compiled version string e.g. "1.80 WIP" (essentially the value for IMGUI_VERSION from the compiled version of imgui.cpp)
	// Styles
	@(link_name="ImGui_StyleColorsDark") StyleColorsDark :: proc "c" (dst: ^Style) --- // new, recommended style (default)
	@(link_name="ImGui_StyleColorsLight") StyleColorsLight :: proc "c" (dst: ^Style) --- // best used with borders and a custom, thicker font
	@(link_name="ImGui_StyleColorsClassic") StyleColorsClassic :: proc "c" (dst: ^Style) --- // classic imgui style
	// Windows
	...
	@(link_name="ImGui_Begin") Begin :: proc "c" (name: cstring, p_open: ^c.bool, flags: WindowFlags) -> c.bool ---
	...
	//    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX function
	//    returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
	@(link_name="ImGui_BeginChild") BeginChild :: proc "c" (str_id: cstring, size: Vec2, border: c.bool, flags: WindowFlags) -> c.bool ---
	@(link_name="ImGui_BeginChildID") BeginChildID :: proc "c" (id: ID, size: Vec2, border: c.bool, flags: WindowFlags) -> c.bool ---
	@(link_name="ImGui_EndChild") EndChild :: proc "c" () ---
	// Windows Utilities
	// - 'current window' = the window we are appending into while inside a Begin()/End() block. 'next window' = next window we will Begin() into.
	@(link_name="ImGui_IsWindowAppearing") IsWindowAppearing :: proc "c" () -> c.bool ---
	@(link_name="ImGui_IsWindowCollapsed") IsWindowCollapsed :: proc "c" () -> c.bool ---
	@(link_name="ImGui_IsWindowFocused") IsWindowFocused :: proc "c" (flags: FocusedFlags) -> c.bool --- // is current window focused? or its root/child, depending on flags. see flags for options.
	@(link_name="ImGui_IsWindowHovered") IsWindowHovered :: proc "c" (flags: HoveredFlags) -> c.bool --- // is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!
	@(link_name="ImGui_GetWindowDrawList") GetWindowDrawList :: proc "c" () -> ^DrawList --- // get draw list associated to the current window, to append your own drawing primitives
	@(link_name="ImGui_GetWindowDpiScale") GetWindowDpiScale :: proc "c" () -> f32 --- // get DPI scale currently associated to the current window's viewport.
	@(link_name="ImGui_GetWindowPos") GetWindowPos :: proc "c" () -> Vec2 --- // get current window position in screen space (note: it is unlikely you need to use this. Consider using current layout pos instead, GetScreenCursorPos())
	@(link_name="ImGui_GetWindowSize") GetWindowSize :: proc "c" () -> Vec2 --- // get current window size (note: it is unlikely you need to use this. Consider using GetScreenCursorPos() and e.g. GetContentRegionAvail() instead)
	@(link_name="ImGui_GetWindowWidth") GetWindowWidth :: proc "c" () -> f32 --- // get current window width (shortcut for GetWindowSize().x)
	@(link_name="ImGui_GetWindowHeight") GetWindowHeight :: proc "c" () -> f32 --- // get current window height (shortcut for GetWindowSize().y)
	@(link_name="ImGui_GetWindowViewport") GetWindowViewport :: proc "c" () -> ^Viewport --- // get viewport currently associated to the current window.
	// Window manipulation
	// - Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).
	@(link_name="ImGui_SetNextWindowPos") SetNextWindowPos :: proc "c" (pos: Vec2, cond: Cond) --- // Implied pivot = ImVec2(0, 0)
	@(link_name="ImGui_SetNextWindowPosEx") SetNextWindowPosEx :: proc "c" (pos: Vec2, cond: Cond, pivot: Vec2) --- // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
	@(link_name="ImGui_SetNextWindowSize") SetNextWindowSize :: proc "c" (size: Vec2, cond: Cond) --- // set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
	@(link_name="ImGui_SetNextWindowSizeConstraints") SetNextWindowSizeConstraints :: proc "c" (size_min: Vec2, size_max: Vec2, custom_callback: SizeCallback, custom_callback_data: rawptr) --- // set next window size limits. use -1,-1 on either X/Y axis to preserve the current size. Sizes will be rounded down. Use callback to apply non-trivial programmatic constraints.
	@(link_name="ImGui_SetNextWindowContentSize") SetNextWindowContentSize :: proc "c" (size: Vec2) --- // set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
	@(link_name="ImGui_SetNextWindowCollapsed") SetNextWindowCollapsed :: proc "c" (collapsed: c.bool, cond: Cond) --- // set next window collapsed state. call before Begin()
	@(link_name="ImGui_SetNextWindowFocus") SetNextWindowFocus :: proc "c" () --- // set next window to be focused / top-most. call before Begin()
	@(link_name="ImGui_SetNextWindowScroll") SetNextWindowScroll :: proc "c" (scroll: Vec2) --- // set next window scrolling value (use < 0.0f to not affect a given axis).
	@(link_name="ImGui_SetNextWindowBgAlpha") SetNextWindowBgAlpha :: proc "c" (alpha: f32) --- // set next window background color alpha. helper to easily override the Alpha component of ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use ImGuiWindowFlags_NoBackground.
	@(link_name="ImGui_SetNextWindowViewport") SetNextWindowViewport :: proc "c" (viewport_id: ID) --- // set next window viewport
	@(link_name="ImGui_SetWindowPos") SetWindowPos :: proc "c" (pos: Vec2, cond: Cond) --- // (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
	@(link_name="ImGui_SetWindowSize") SetWindowSize :: proc "c" (size: Vec2, cond: Cond) --- // (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
	@(link_name="ImGui_SetWindowCollapsed") SetWindowCollapsed :: proc "c" (collapsed: c.bool, cond: Cond) --- // (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
	@(link_name="ImGui_SetWindowFocus") SetWindowFocus :: proc "c" () --- // (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
	@(link_name="ImGui_SetWindowFontScale") SetWindowFontScale :: proc "c" (scale: f32) --- // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
	@(link_name="ImGui_SetWindowPosStr") SetWindowPosStr :: proc "c" (name: cstring, pos: Vec2, cond: Cond) --- // set named window position.
	@(link_name="ImGui_SetWindowSizeStr") SetWindowSizeStr :: proc "c" (name: cstring, size: Vec2, cond: Cond) --- // set named window size. set axis to 0.0f to force an auto-fit on this axis.
	@(link_name="ImGui_SetWindowCollapsedStr") SetWindowCollapsedStr :: proc "c" (name: cstring, collapsed: c.bool, cond: Cond) --- // set named window collapsed state
	@(link_name="ImGui_SetWindowFocusStr") SetWindowFocusStr :: proc "c" (name: cstring) --- // set named window to be focused / top-most. use NULL to remove focus.
	// Content region
	// - Retrieve available space from a given point. GetContentRegionAvail() is frequently useful.
	// - Those functions are bound to be redesigned (they are confusing, incomplete and the Min/Max return values are in local window coordinates which increases confusion)
	@(link_name="ImGui_GetContentRegionAvail") GetContentRegionAvail :: proc "c" () -> Vec2 --- // == GetContentRegionMax() - GetCursorPos()
	@(link_name="ImGui_GetContentRegionMax") GetContentRegionMax :: proc "c" () -> Vec2 --- // current content boundaries (typically window boundaries including scrolling, or current column boundaries), in windows coordinates
	@(link_name="ImGui_GetWindowContentRegionMin") GetWindowContentRegionMin :: proc "c" () -> Vec2 --- // content boundaries min for the full window (roughly (0,0)-Scroll), in window coordinates
	@(link_name="ImGui_GetWindowContentRegionMax") GetWindowContentRegionMax :: proc "c" () -> Vec2 --- // content boundaries max for the full window (roughly (0,0)+Size-Scroll) where Size can be overridden with SetNextWindowContentSize(), in window coordinates
	// Windows Scrolling
	// - Any change of Scroll will be applied at the beginning of next frame in the first call to Begin().
	// - You may instead use SetNextWindowScroll() prior to calling Begin() to avoid this delay, as an alternative to using SetScrollX()/SetScrollY().
	@(link_name="ImGui_GetScrollX") GetScrollX :: proc "c" () -> f32 --- // get scrolling amount [0 .. GetScrollMaxX()]
	@(link_name="ImGui_GetScrollY") GetScrollY :: proc "c" () -> f32 --- // get scrolling amount [0 .. GetScrollMaxY()]
	@(link_name="ImGui_SetScrollX") SetScrollX :: proc "c" (scroll_x: f32) --- // set scrolling amount [0 .. GetScrollMaxX()]
	@(link_name="ImGui_SetScrollY") SetScrollY :: proc "c" (scroll_y: f32) --- // set scrolling amount [0 .. GetScrollMaxY()]
	@(link_name="ImGui_GetScrollMaxX") GetScrollMaxX :: proc "c" () -> f32 --- // get maximum scrolling amount ~~ ContentSize.x - WindowSize.x - DecorationsSize.x
	@(link_name="ImGui_GetScrollMaxY") GetScrollMaxY :: proc "c" () -> f32 --- // get maximum scrolling amount ~~ ContentSize.y - WindowSize.y - DecorationsSize.y
	@(link_name="ImGui_SetScrollHereX") SetScrollHereX :: proc "c" (center_x_ratio: f32) --- // adjust scrolling amount to make current cursor position visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
	@(link_name="ImGui_SetScrollHereY") SetScrollHereY :: proc "c" (center_y_ratio: f32) --- // adjust scrolling amount to make current cursor position visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
	@(link_name="ImGui_SetScrollFromPosX") SetScrollFromPosX :: proc "c" (local_x: f32, center_x_ratio: f32) --- // adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
	@(link_name="ImGui_SetScrollFromPosY") SetScrollFromPosY :: proc "c" (local_y: f32, center_y_ratio: f32) --- // adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
	...
	@(link_name="ImGui_PushStyleVar") PushStyleVar :: proc "c" (idx: StyleVar, val: f32) --- // modify a style float variable. always use this if you modify the style after NewFrame().
	@(link_name="ImGui_PushStyleVarImVec2") PushStyleVarImVec2 :: proc "c" (idx: StyleVar, val: Vec2) --- // modify a style ImVec2 variable. always use this if you modify the style after NewFrame().
	@(link_name="ImGui_PopStyleVar") PopStyleVar :: proc "c" () --- // Implied count = 1
	@(link_name="ImGui_PopStyleVarEx") PopStyleVarEx :: proc "c" (count: c.int) ---
	@(link_name="ImGui_PushTabStop") PushTabStop :: proc "c" (tab_stop: c.bool) --- // == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default but you can disable it for certain widgets
	@(link_name="ImGui_PopTabStop") PopTabStop :: proc "c" () ---
	@(link_name="ImGui_PushButtonRepeat") PushButtonRepeat :: proc "c" (repeat: c.bool) --- // in 'repeat' mode, Button*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button() to tell if the button is held in the current frame.
	@(link_name="ImGui_PopButtonRepeat") PopButtonRepeat :: proc "c" () ---
	// Parameters stacks (current window)
	@(link_name="ImGui_PushItemWidth") PushItemWidth :: proc "c" (item_width: f32) --- // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side).
	@(link_name="ImGui_PopItemWidth") PopItemWidth :: proc "c" () ---
	@(link_name="ImGui_SetNextItemWidth") SetNextItemWidth :: proc "c" (item_width: f32) --- // set width of the _next_ common large "item+label" widget. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side)
	@(link_name="ImGui_CalcItemWidth") CalcItemWidth :: proc "c" () -> f32 --- // width of item given pushed settings and current cursor position. NOT necessarily the width of last item unlike most 'Item' functions.
	@(link_name="ImGui_PushTextWrapPos") PushTextWrapPos :: proc "c" (wrap_local_pos_x: f32) --- // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local space
	...
	@(link_name="ImGui_Separator") Separator :: proc "c" () --- // separator, generally horizontal. inside a menu bar or in horizontal layout mode, this becomes a vertical separator.
	@(link_name="ImGui_SameLine") SameLine :: proc "c" () --- // Implied offset_from_start_x = 0.0f, spacing = -1.0f
	@(link_name="ImGui_SameLineEx") SameLineEx :: proc "c" (offset_from_start_x: f32, spacing: f32) --- // call between widgets or groups to layout them horizontally. X position given in window coordinates.
	@(link_name="ImGui_NewLine") NewLine :: proc "c" () --- // undo a SameLine() or force a new line when in a horizontal-layout context.
	@(link_name="ImGui_Spacing") Spacing :: proc "c" () --- // add vertical spacing.
	@(link_name="ImGui_Dummy") Dummy :: proc "c" (size: Vec2) --- // add a dummy item of given size. unlike InvisibleButton(), Dummy() won't take the mouse click or be navigable into.
	@(link_name="ImGui_Indent") Indent :: proc "c" () --- // Implied indent_w = 0.0f
	@(link_name="ImGui_IndentEx") IndentEx :: proc "c" (indent_w: f32) --- // move content position toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
	@(link_name="ImGui_Unindent") Unindent :: proc "c" () --- // Implied indent_w = 0.0f
	@(link_name="ImGui_UnindentEx") UnindentEx :: proc "c" (indent_w: f32) --- // move content position back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
	@(link_name="ImGui_BeginGroup") BeginGroup :: proc "c" () --- // lock horizontal starting position
	@(link_name="ImGui_EndGroup") EndGroup :: proc "c" () --- // unlock horizontal starting position + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
	@(link_name="ImGui_GetCursorPos") GetCursorPos :: proc "c" () -> Vec2 --- // cursor position in window coordinates (relative to window position)
	@(link_name="ImGui_GetCursorPosX") GetCursorPosX :: proc "c" () -> f32 --- //   (some functions are using window-relative coordinates, such as: GetCursorPos, GetCursorStartPos, GetContentRegionMax, GetWindowContentRegion* etc.
	@(link_name="ImGui_GetCursorPosY") GetCursorPosY :: proc "c" () -> f32 --- //    other functions such as GetCursorScreenPos or everything in ImDrawList::
	@(link_name="ImGui_SetCursorPos") SetCursorPos :: proc "c" (local_pos: Vec2) --- //    are using the main, absolute coordinate system.
	@(link_name="ImGui_SetCursorPosX") SetCursorPosX :: proc "c" (local_x: f32) --- //    GetWindowPos() + GetCursorPos() == GetCursorScreenPos() etc.)
	@(link_name="ImGui_SetCursorPosY") SetCursorPosY :: proc "c" (local_y: f32) --- //
	@(link_name="ImGui_GetCursorStartPos") GetCursorStartPos :: proc "c" () -> Vec2 --- // initial cursor position in window coordinates
	@(link_name="ImGui_GetCursorScreenPos") GetCursorScreenPos :: proc "c" () -> Vec2 --- // cursor position in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->Pos == (0,0) in single viewport mode, and bottom-right == GetMainViewport()->Pos+Size == io.DisplaySize in single-viewport mode.
	@(link_name="ImGui_SetCursorScreenPos") SetCursorScreenPos :: proc "c" (pos: Vec2) --- // cursor position in absolute coordinates
	@(link_name="ImGui_AlignTextToFramePadding") AlignTextToFramePadding :: proc "c" () --- // vertically align upcoming text baseline to FramePadding.y so that it will align properly to regularly framed items (call if you have text on a line before a framed item)
	@(link_name="ImGui_GetTextLineHeight") GetTextLineHeight :: proc "c" () -> f32 --- // ~ FontSize
	@(link_name="ImGui_GetTextLineHeightWithSpacing") GetTextLineHeightWithSpacing :: proc "c" () -> f32 --- // ~ FontSize + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of text)
	@(link_name="ImGui_GetFrameHeight") GetFrameHeight :: proc "c" () -> f32 --- // ~ FontSize + style.FramePadding.y * 2
	@(link_name="ImGui_GetFrameHeightWithSpacing") GetFrameHeightWithSpacing :: proc "c" () -> f32 --- // ~ FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of framed widgets)
	// ID stack/scopes
	...
	@(link_name="ImGui_PushID") PushID :: proc "c" (str_id: cstring) --- // push string into the ID stack (will hash string).
	@(link_name="ImGui_PushIDStr") PushIDStr :: proc "c" (str_id_begin: cstring, str_id_end: cstring) --- // push string into the ID stack (will hash string).
	@(link_name="ImGui_PushIDPtr") PushIDPtr :: proc "c" (ptr_id: rawptr) --- // push pointer into the ID stack (will hash pointer).
	@(link_name="ImGui_PushIDInt") PushIDInt :: proc "c" (int_id: c.int) --- // push integer into the ID stack (will hash integer).
	@(link_name="ImGui_PopID") PopID :: proc "c" () --- // pop from the ID stack.
	@(link_name="ImGui_GetID") GetID :: proc "c" (str_id: cstring) -> ID --- // calculate unique ID (hash of whole ID stack + given parameter). e.g. if you want to query into ImGuiStorage yourself
	...
	// Widgets: Text
	@(link_name="ImGui_TextUnformatted") TextUnformatted :: proc "c" (text: cstring) --- // Implied text_end = NULL
	@(link_name="ImGui_TextUnformattedEx") TextUnformattedEx :: proc "c" (text: cstring, text_end: cstring) --- // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
	@(link_name="ImGui_Text") Text :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // formatted text
	// @(link_name="ImGui_TextV") TextV :: proc "c" (fmt: cstring, args: libc.va_list) ---
	@(link_name="ImGui_TextColored") TextColored :: proc "c" (col: Vec4, fmt: cstring, #c_vararg args: ..any) --- // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
	// @(link_name="ImGui_TextColoredV") TextColoredV :: proc "c" (col: Vec4, fmt: cstring, args: libc.va_list) ---
	@(link_name="ImGui_TextDisabled") TextDisabled :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
	// @(link_name="ImGui_TextDisabledV") TextDisabledV :: proc "c" (fmt: cstring, args: libc.va_list) ---
	@(link_name="ImGui_TextWrapped") TextWrapped :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
	// @(link_name="ImGui_TextWrappedV") TextWrappedV :: proc "c" (fmt: cstring, args: libc.va_list) ---
	@(link_name="ImGui_LabelText") LabelText :: proc "c" (label: cstring, fmt: cstring, #c_vararg args: ..any) --- // display text+label aligned the same way as value+label widgets
	// @(link_name="ImGui_LabelTextV") LabelTextV :: proc "c" (label: cstring, fmt: cstring, args: libc.va_list) ---
	@(link_name="ImGui_BulletText") BulletText :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // shortcut for Bullet()+Text()
	// @(link_name="ImGui_BulletTextV") BulletTextV :: proc "c" (fmt: cstring, args: libc.va_list) ---
	@(link_name="ImGui_SeparatorText") SeparatorText :: proc "c" (label: cstring) --- // currently: formatted text with an horizontal line
	// Widgets: Main
	// - Most widgets return true when the value has been changed or when pressed/selected
	// - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget state.
	@(link_name="ImGui_Button") Button :: proc "c" (label: cstring) -> c.bool --- // Implied size = ImVec2(0, 0)
	@(link_name="ImGui_ButtonEx") ButtonEx :: proc "c" (label: cstring, size: Vec2) -> c.bool --- // button
	@(link_name="ImGui_SmallButton") SmallButton :: proc "c" (label: cstring) -> c.bool --- // button with FramePadding=(0,0) to easily embed within text
	@(link_name="ImGui_InvisibleButton") InvisibleButton :: proc "c" (str_id: cstring, size: Vec2, flags: ButtonFlags) -> c.bool --- // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
	@(link_name="ImGui_ArrowButton") ArrowButton :: proc "c" (str_id: cstring, dir: Dir) -> c.bool --- // square button with an arrow shape
	@(link_name="ImGui_Checkbox") Checkbox :: proc "c" (label: cstring, v: ^c.bool) -> c.bool ---
	@(link_name="ImGui_CheckboxFlagsIntPtr") CheckboxFlagsIntPtr :: proc "c" (label: cstring, flags: ^c.int, flags_value: c.int) -> c.bool ---
	@(link_name="ImGui_CheckboxFlagsUintPtr") CheckboxFlagsUintPtr :: proc "c" (label: cstring, flags: ^c.uint, flags_value: c.uint) -> c.bool ---
	@(link_name="ImGui_RadioButton") RadioButton :: proc "c" (label: cstring, active: c.bool) -> c.bool --- // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
	@(link_name="ImGui_RadioButtonIntPtr") RadioButtonIntPtr :: proc "c" (label: cstring, v: ^c.int, v_button: c.int) -> c.bool --- // shortcut to handle the above pattern when value is an integer
	@(link_name="ImGui_ProgressBar") ProgressBar :: proc "c" (fraction: f32, size_arg: Vec2, overlay: cstring) ---
	@(link_name="ImGui_Bullet") Bullet :: proc "c" () --- // draw a small circle + keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
	// Widgets: Images
	// - Read about ImTextureID here: https://github.com/ocornut/imgui/wiki/Image-Loading-and-Displaying-Examples
	@(link_name="ImGui_Image") Image :: proc "c" (user_texture_id: TextureID, size: Vec2) --- // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), tint_col = ImVec4(1, 1, 1, 1), border_col = ImVec4(0, 0, 0, 0)
	...
	@(link_name="ImGui_ImageButton") ImageButton :: proc "c" (str_id: cstring, user_texture_id: TextureID, size: Vec2) -> c.bool --- // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
	@(link_name="ImGui_ImageButtonEx") ImageButtonEx :: proc "c" (str_id: cstring, user_texture_id: TextureID, size: Vec2, uv0: Vec2, uv1: Vec2, bg_col: Vec4, tint_col: Vec4) -> c.bool ---
	// Widgets: Combo Box (Dropdown)
	// - The BeginCombo()/EndCombo() api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() items.
	// - The old Combo() api are helpers over BeginCombo()/EndCombo() which are kept available for convenience purpose. This is analogous to how ListBox are created.
	@(link_name="ImGui_BeginCombo") BeginCombo :: proc "c" (label: cstring, preview_value: cstring, flags: ComboFlags) -> c.bool ---
	@(link_name="ImGui_EndCombo") EndCombo :: proc "c" () --- // only call EndCombo() if BeginCombo() returns true!
	@(link_name="ImGui_ComboChar") ComboChar :: proc "c" (label: cstring, current_item: ^c.int, items: [^]cstring, items_count: c.int) -> c.bool --- // Implied popup_max_height_in_items = -1
	@(link_name="ImGui_ComboCharEx") ComboCharEx :: proc "c" (label: cstring, current_item: ^c.int, items: [^]cstring, items_count: c.int, popup_max_height_in_items: c.int) -> c.bool ---
	@(link_name="ImGui_Combo") Combo :: proc "c" (label: cstring, current_item: ^c.int, items_separated_by_zeros: cstring) -> c.bool --- // Implied popup_max_height_in_items = -1
	@(link_name="ImGui_ComboEx") ComboEx :: proc "c" (label: cstring, current_item: ^c.int, items_separated_by_zeros: cstring, popup_max_height_in_items: c.int) -> c.bool --- // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
	@(link_name="ImGui_ComboCallback") ComboCallback :: proc "c" (label: cstring, current_item: ^c.int, getter: proc "c" (user_data: rawptr, idx: c.int) -> cstring, user_data: rawptr, items_count: c.int) -> c.bool --- // Implied popup_max_height_in_items = -1
	@(link_name="ImGui_ComboCallbackEx") ComboCallbackEx :: proc "c" (label: cstring, current_item: ^c.int, getter: proc "c" (user_data: rawptr, idx: c.int) -> cstring, user_data: rawptr, items_count: c.int, popup_max_height_in_items: c.int) -> c.bool ---
	...
	@(link_name="ImGui_DragFloat") DragFloat :: proc "c" (label: cstring, v: ^f32) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	@(link_name="ImGui_DragFloatEx") DragFloatEx :: proc "c" (label: cstring, v: ^f32, v_speed: f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragFloat2") DragFloat2 :: proc "c" (label: cstring, v: ^[2]f32) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	@(link_name="ImGui_DragFloat2Ex") DragFloat2Ex :: proc "c" (label: cstring, v: ^[2]f32, v_speed: f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragFloat3") DragFloat3 :: proc "c" (label: cstring, v: ^[3]f32) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	@(link_name="ImGui_DragFloat3Ex") DragFloat3Ex :: proc "c" (label: cstring, v: ^[3]f32, v_speed: f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragFloat4") DragFloat4 :: proc "c" (label: cstring, v: ^[4]f32) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	@(link_name="ImGui_DragFloat4Ex") DragFloat4Ex :: proc "c" (label: cstring, v: ^[4]f32, v_speed: f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragFloatRange2") DragFloatRange2 :: proc "c" (label: cstring, v_current_min: ^f32, v_current_max: ^f32) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", format_max = NULL, flags = 0
	@(link_name="ImGui_DragFloatRange2Ex") DragFloatRange2Ex :: proc "c" (label: cstring, v_current_min: ^f32, v_current_max: ^f32, v_speed: f32, v_min: f32, v_max: f32, format: cstring, format_max: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragInt") DragInt :: proc "c" (label: cstring, v: ^c.int) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	@(link_name="ImGui_DragIntEx") DragIntEx :: proc "c" (label: cstring, v: ^c.int, v_speed: f32, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragInt2") DragInt2 :: proc "c" (label: cstring, v: ^[2]c.int) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	@(link_name="ImGui_DragInt2Ex") DragInt2Ex :: proc "c" (label: cstring, v: ^[2]c.int, v_speed: f32, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragInt3") DragInt3 :: proc "c" (label: cstring, v: ^[3]c.int) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	@(link_name="ImGui_DragInt3Ex") DragInt3Ex :: proc "c" (label: cstring, v: ^[3]c.int, v_speed: f32, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragInt4") DragInt4 :: proc "c" (label: cstring, v: ^[4]c.int) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	@(link_name="ImGui_DragInt4Ex") DragInt4Ex :: proc "c" (label: cstring, v: ^[4]c.int, v_speed: f32, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragIntRange2") DragIntRange2 :: proc "c" (label: cstring, v_current_min: ^c.int, v_current_max: ^c.int) -> c.bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", format_max = NULL, flags = 0
	@(link_name="ImGui_DragIntRange2Ex") DragIntRange2Ex :: proc "c" (label: cstring, v_current_min: ^c.int, v_current_max: ^c.int, v_speed: f32, v_min: c.int, v_max: c.int, format: cstring, format_max: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragScalar") DragScalar :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr) -> c.bool --- // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
	@(link_name="ImGui_DragScalarEx") DragScalarEx :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, v_speed: f32, p_min: rawptr, p_max: rawptr, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_DragScalarN") DragScalarN :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int) -> c.bool --- // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
	@(link_name="ImGui_DragScalarNEx") DragScalarNEx :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int, v_speed: f32, p_min: rawptr, p_max: rawptr, format: cstring, flags: SliderFlags) -> c.bool ---
	...
	//   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
	@(link_name="ImGui_SliderFloat") SliderFloat :: proc "c" (label: cstring, v: ^f32, v_min: f32, v_max: f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_SliderFloatEx") SliderFloatEx :: proc "c" (label: cstring, v: ^f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool --- // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
	@(link_name="ImGui_SliderFloat2") SliderFloat2 :: proc "c" (label: cstring, v: ^[2]f32, v_min: f32, v_max: f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_SliderFloat2Ex") SliderFloat2Ex :: proc "c" (label: cstring, v: ^[2]f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderFloat3") SliderFloat3 :: proc "c" (label: cstring, v: ^[3]f32, v_min: f32, v_max: f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_SliderFloat3Ex") SliderFloat3Ex :: proc "c" (label: cstring, v: ^[3]f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderFloat4") SliderFloat4 :: proc "c" (label: cstring, v: ^[4]f32, v_min: f32, v_max: f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_SliderFloat4Ex") SliderFloat4Ex :: proc "c" (label: cstring, v: ^[4]f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderAngle") SliderAngle :: proc "c" (label: cstring, v_rad: ^f32) -> c.bool --- // Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f, format = "%.0f deg", flags = 0
	@(link_name="ImGui_SliderAngleEx") SliderAngleEx :: proc "c" (label: cstring, v_rad: ^f32, v_degrees_min: f32, v_degrees_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderInt") SliderInt :: proc "c" (label: cstring, v: ^c.int, v_min: c.int, v_max: c.int) -> c.bool --- // Implied format = "%d", flags = 0
	@(link_name="ImGui_SliderIntEx") SliderIntEx :: proc "c" (label: cstring, v: ^c.int, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderInt2") SliderInt2 :: proc "c" (label: cstring, v: ^[2]c.int, v_min: c.int, v_max: c.int) -> c.bool --- // Implied format = "%d", flags = 0
	@(link_name="ImGui_SliderInt2Ex") SliderInt2Ex :: proc "c" (label: cstring, v: ^[2]c.int, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderInt3") SliderInt3 :: proc "c" (label: cstring, v: ^[3]c.int, v_min: c.int, v_max: c.int) -> c.bool --- // Implied format = "%d", flags = 0
	@(link_name="ImGui_SliderInt3Ex") SliderInt3Ex :: proc "c" (label: cstring, v: ^[3]c.int, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderInt4") SliderInt4 :: proc "c" (label: cstring, v: ^[4]c.int, v_min: c.int, v_max: c.int) -> c.bool --- // Implied format = "%d", flags = 0
	@(link_name="ImGui_SliderInt4Ex") SliderInt4Ex :: proc "c" (label: cstring, v: ^[4]c.int, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderScalar") SliderScalar :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr) -> c.bool --- // Implied format = NULL, flags = 0
	@(link_name="ImGui_SliderScalarEx") SliderScalarEx :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_SliderScalarN") SliderScalarN :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int, p_min: rawptr, p_max: rawptr) -> c.bool --- // Implied format = NULL, flags = 0
	@(link_name="ImGui_SliderScalarNEx") SliderScalarNEx :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int, p_min: rawptr, p_max: rawptr, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_VSliderFloat") VSliderFloat :: proc "c" (label: cstring, size: Vec2, v: ^f32, v_min: f32, v_max: f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_VSliderFloatEx") VSliderFloatEx :: proc "c" (label: cstring, size: Vec2, v: ^f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_VSliderInt") VSliderInt :: proc "c" (label: cstring, size: Vec2, v: ^c.int, v_min: c.int, v_max: c.int) -> c.bool --- // Implied format = "%d", flags = 0
	@(link_name="ImGui_VSliderIntEx") VSliderIntEx :: proc "c" (label: cstring, size: Vec2, v: ^c.int, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> c.bool ---
	@(link_name="ImGui_VSliderScalar") VSliderScalar :: proc "c" (label: cstring, size: Vec2, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr) -> c.bool --- // Implied format = NULL, flags = 0
	@(link_name="ImGui_VSliderScalarEx") VSliderScalarEx :: proc "c" (label: cstring, size: Vec2, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring, flags: SliderFlags) -> c.bool ---
	// Widgets: Input with Keyboard
	// - If you want to use InputText() with std::string or any custom dynamic string type, see misc/cpp/imgui_stdlib.h and comments in imgui_demo.cpp.
	// - Most of the ImGuiInputTextFlags flags are only useful for InputText() and not for InputFloatX, InputIntX, InputDouble etc.
	@(link_name="ImGui_InputText") InputText :: proc "c" (label: cstring, buf: cstring, buf_size: c.size_t, flags: InputTextFlags) -> c.bool --- // Implied callback = NULL, user_data = NULL
	@(link_name="ImGui_InputTextEx") InputTextEx :: proc "c" (label: cstring, buf: cstring, buf_size: c.size_t, flags: InputTextFlags, callback: InputTextCallback, user_data: rawptr) -> c.bool ---
	@(link_name="ImGui_InputTextMultiline") InputTextMultiline :: proc "c" (label: cstring, buf: cstring, buf_size: c.size_t) -> c.bool --- // Implied size = ImVec2(0, 0), flags = 0, callback = NULL, user_data = NULL
	@(link_name="ImGui_InputTextMultilineEx") InputTextMultilineEx :: proc "c" (label: cstring, buf: cstring, buf_size: c.size_t, size: Vec2, flags: InputTextFlags, callback: InputTextCallback, user_data: rawptr) -> c.bool ---
	@(link_name="ImGui_InputTextWithHint") InputTextWithHint :: proc "c" (label: cstring, hint: cstring, buf: cstring, buf_size: c.size_t, flags: InputTextFlags) -> c.bool --- // Implied callback = NULL, user_data = NULL
	@(link_name="ImGui_InputTextWithHintEx") InputTextWithHintEx :: proc "c" (label: cstring, hint: cstring, buf: cstring, buf_size: c.size_t, flags: InputTextFlags, callback: InputTextCallback, user_data: rawptr) -> c.bool ---
	@(link_name="ImGui_InputFloat") InputFloat :: proc "c" (label: cstring, v: ^f32) -> c.bool --- // Implied step = 0.0f, step_fast = 0.0f, format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloatEx") InputFloatEx :: proc "c" (label: cstring, v: ^f32, step: f32, step_fast: f32, format: cstring, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputFloat2") InputFloat2 :: proc "c" (label: cstring, v: ^[2]f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloat2Ex") InputFloat2Ex :: proc "c" (label: cstring, v: ^[2]f32, format: cstring, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputFloat3") InputFloat3 :: proc "c" (label: cstring, v: ^[3]f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloat3Ex") InputFloat3Ex :: proc "c" (label: cstring, v: ^[3]f32, format: cstring, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputFloat4") InputFloat4 :: proc "c" (label: cstring, v: ^[4]f32) -> c.bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloat4Ex") InputFloat4Ex :: proc "c" (label: cstring, v: ^[4]f32, format: cstring, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputInt") InputInt :: proc "c" (label: cstring, v: ^c.int) -> c.bool --- // Implied step = 1, step_fast = 100, flags = 0
	@(link_name="ImGui_InputIntEx") InputIntEx :: proc "c" (label: cstring, v: ^c.int, step: c.int, step_fast: c.int, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputInt2") InputInt2 :: proc "c" (label: cstring, v: ^[2]c.int, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputInt3") InputInt3 :: proc "c" (label: cstring, v: ^[3]c.int, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputInt4") InputInt4 :: proc "c" (label: cstring, v: ^[4]c.int, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputDouble") InputDouble :: proc "c" (label: cstring, v: ^f64) -> c.bool --- // Implied step = 0.0, step_fast = 0.0, format = "%.6f", flags = 0
	@(link_name="ImGui_InputDoubleEx") InputDoubleEx :: proc "c" (label: cstring, v: ^f64, step: f64, step_fast: f64, format: cstring, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputScalar") InputScalar :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr) -> c.bool --- // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
	@(link_name="ImGui_InputScalarEx") InputScalarEx :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, p_step: rawptr, p_step_fast: rawptr, format: cstring, flags: InputTextFlags) -> c.bool ---
	@(link_name="ImGui_InputScalarN") InputScalarN :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int) -> c.bool --- // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
	@(link_name="ImGui_InputScalarNEx") InputScalarNEx :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int, p_step: rawptr, p_step_fast: rawptr, format: cstring, flags: InputTextFlags) -> c.bool ---
	// Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
	// - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
	// - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.x
	@(link_name="ImGui_ColorEdit3") ColorEdit3 :: proc "c" (label: cstring, col: ^[3]f32, flags: ColorEditFlags) -> c.bool ---
	@(link_name="ImGui_ColorEdit4") ColorEdit4 :: proc "c" (label: cstring, col: ^[4]f32, flags: ColorEditFlags) -> c.bool ---
	@(link_name="ImGui_ColorPicker3") ColorPicker3 :: proc "c" (label: cstring, col: ^[3]f32, flags: ColorEditFlags) -> c.bool ---
	@(link_name="ImGui_ColorPicker4") ColorPicker4 :: proc "c" (label: cstring, col: ^[4]f32, flags: ColorEditFlags, ref_col: ^f32) -> c.bool ---
	@(link_name="ImGui_ColorButton") ColorButton :: proc "c" (desc_id: cstring, col: Vec4, flags: ColorEditFlags) -> c.bool --- // Implied size = ImVec2(0, 0)
	@(link_name="ImGui_ColorButtonEx") ColorButtonEx :: proc "c" (desc_id: cstring, col: Vec4, flags: ColorEditFlags, size: Vec2) -> c.bool --- // display a color square/button, hover for details, return true when pressed.
	@(link_name="ImGui_SetColorEditOptions") SetColorEditOptions :: proc "c" (flags: ColorEditFlags) --- // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
	// Widgets: Trees
	// - TreeNode functions return true when the node is open, in which case you need to also call TreePop() when you are finished displaying the tree node contents.
	@(link_name="ImGui_TreeNode") TreeNode :: proc "c" (label: cstring) -> c.bool ---
	@(link_name="ImGui_TreeNodeStr") TreeNodeStr :: proc "c" (str_id: cstring, fmt: cstring, #c_vararg args: ..any) -> c.bool --- // helper variation to easily decorelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
	@(link_name="ImGui_TreeNodePtr") TreeNodePtr :: proc "c" (ptr_id: rawptr, fmt: cstring, #c_vararg args: ..any) -> c.bool --- // "
	// @(link_name="ImGui_TreeNodeV") TreeNodeV :: proc "c" (str_id: cstring, fmt: cstring, args: libc.va_list) -> c.bool ---
	// @(link_name="ImGui_TreeNodeVPtr") TreeNodeVPtr :: proc "c" (ptr_id: rawptr, fmt: cstring, args: libc.va_list) -> c.bool ---
	@(link_name="ImGui_TreeNodeEx") TreeNodeEx :: proc "c" (label: cstring, flags: TreeNodeFlags) -> c.bool ---
	@(link_name="ImGui_TreeNodeExStr") TreeNodeExStr :: proc "c" (str_id: cstring, flags: TreeNodeFlags, fmt: cstring, #c_vararg args: ..any) -> c.bool ---
	@(link_name="ImGui_TreeNodeExPtr") TreeNodeExPtr :: proc "c" (ptr_id: rawptr, flags: TreeNodeFlags, fmt: cstring, #c_vararg args: ..any) -> c.bool ---
	// @(link_name="ImGui_TreeNodeExV") TreeNodeExV :: proc "c" (str_id: cstring, flags: TreeNodeFlags, fmt: cstring, args: libc.va_list) -> c.bool ---
	// @(link_name="ImGui_TreeNodeExVPtr") TreeNodeExVPtr :: proc "c" (ptr_id: rawptr, flags: TreeNodeFlags, fmt: cstring, args: libc.va_list) -> c.bool ---
	@(link_name="ImGui_TreePush") TreePush :: proc "c" (str_id: cstring) --- // ~ Indent()+PushId(). Already called by TreeNode() when returning true, but you can call TreePush/TreePop yourself if desired.
	@(link_name="ImGui_TreePushPtr") TreePushPtr :: proc "c" (ptr_id: rawptr) --- // "
	@(link_name="ImGui_TreePop") TreePop :: proc "c" () --- // ~ Unindent()+PopId()
	@(link_name="ImGui_GetTreeNodeToLabelSpacing") GetTreeNodeToLabelSpacing :: proc "c" () -> f32 --- // horizontal distance preceding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeNode
	@(link_name="ImGui_CollapsingHeader") CollapsingHeader :: proc "c" (label: cstring, flags: TreeNodeFlags) -> c.bool --- // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
	@(link_name="ImGui_CollapsingHeaderBoolPtr") CollapsingHeaderBoolPtr :: proc "c" (label: cstring, p_visible: ^c.bool, flags: TreeNodeFlags) -> c.bool --- // when 'p_visible != NULL': if '*p_visible==true' display an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't display the header.
	@(link_name="ImGui_SetNextItemOpen") SetNextItemOpen :: proc "c" (is_open: c.bool, cond: Cond) --- // set next TreeNode/CollapsingHeader open state.
	// Widgets: Selectables
	// - A selectable highlights when hovered, and can display another color when selected.
	// - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
	@(link_name="ImGui_Selectable") Selectable :: proc "c" (label: cstring) -> c.bool --- // Implied selected = false, flags = 0, size = ImVec2(0, 0)
	@(link_name="ImGui_SelectableEx") SelectableEx :: proc "c" (label: cstring, selected: c.bool, flags: SelectableFlags, size: Vec2) -> c.bool --- // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
	@(link_name="ImGui_SelectableBoolPtr") SelectableBoolPtr :: proc "c" (label: cstring, p_selected: ^c.bool, flags: SelectableFlags) -> c.bool --- // Implied size = ImVec2(0, 0)
	@(link_name="ImGui_SelectableBoolPtrEx") SelectableBoolPtrEx :: proc "c" (label: cstring, p_selected: ^c.bool, flags: SelectableFlags, size: Vec2) -> c.bool --- // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
	// Widgets: List Boxes
	...
	@(link_name="ImGui_BeginListBox") BeginListBox :: proc "c" (label: cstring, size: Vec2) -> c.bool --- // open a framed scrolling region
	@(link_name="ImGui_EndListBox") EndListBox :: proc "c" () --- // only call EndListBox() if BeginListBox() returned true!
	@(link_name="ImGui_ListBox") ListBox :: proc "c" (label: cstring, current_item: ^c.int, items: [^]cstring, items_count: c.int, height_in_items: c.int) -> c.bool ---
	@(link_name="ImGui_ListBoxCallback") ListBoxCallback :: proc "c" (label: cstring, current_item: ^c.int, getter: proc "c" (user_data: rawptr, idx: c.int) -> cstring, user_data: rawptr, items_count: c.int) -> c.bool --- // Implied height_in_items = -1
	@(link_name="ImGui_ListBoxCallbackEx") ListBoxCallbackEx :: proc "c" (label: cstring, current_item: ^c.int, getter: proc "c" (user_data: rawptr, idx: c.int) -> cstring, user_data: rawptr, items_count: c.int, height_in_items: c.int) -> c.bool ---
	...
	@(link_name="ImGui_PlotLinesCallback") PlotLinesCallback :: proc "c" (label: cstring, values_getter: proc "c" (data: rawptr, idx: c.int) -> f32, data: rawptr, values_count: c.int) --- // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
	...
	@(link_name="ImGui_PlotHistogram") PlotHistogram :: proc "c" (label: cstring, values: ^f32, values_count: c.int) --- // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
	...
	@(link_name="ImGui_PlotHistogramCallback") PlotHistogramCallback :: proc "c" (label: cstring, values_getter: proc "c" (data: rawptr, idx: c.int) -> f32, data: rawptr, values_count: c.int) --- // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
	...
	@(link_name="ImGui_BeginMenuBar") BeginMenuBar :: proc "c" () -> c.bool --- // append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
	@(link_name="ImGui_EndMenuBar") EndMenuBar :: proc "c" () --- // only call EndMenuBar() if BeginMenuBar() returns true!
	@(link_name="ImGui_BeginMainMenuBar") BeginMainMenuBar :: proc "c" () -> c.bool --- // create and append to a full screen menu-bar.
	@(link_name="ImGui_EndMainMenuBar") EndMainMenuBar :: proc "c" () --- // only call EndMainMenuBar() if BeginMainMenuBar() returns true!
	@(link_name="ImGui_BeginMenu") BeginMenu :: proc "c" (label: cstring) -> c.bool --- // Implied enabled = true
	@(link_name="ImGui_BeginMenuEx") BeginMenuEx :: proc "c" (label: cstring, enabled: c.bool) -> c.bool --- // create a sub-menu entry. only call EndMenu() if this returns true!
	@(link_name="ImGui_EndMenu") EndMenu :: proc "c" () --- // only call EndMenu() if BeginMenu() returns true!
	@(link_name="ImGui_MenuItem") MenuItem :: proc "c" (label: cstring) -> c.bool --- // Implied shortcut = NULL, selected = false, enabled = true
	@(link_name="ImGui_MenuItemEx") MenuItemEx :: proc "c" (label: cstring, shortcut: cstring, selected: c.bool, enabled: c.bool) -> c.bool --- // return true when activated.
	@(link_name="ImGui_MenuItemBoolPtr") MenuItemBoolPtr :: proc "c" (label: cstring, shortcut: cstring, p_selected: ^c.bool, enabled: c.bool) -> c.bool --- // return true when activated + toggle (*p_selected) if p_selected != NULL
	// Tooltips
	...
	@(link_name="ImGui_BeginTooltip") BeginTooltip :: proc "c" () -> c.bool --- // begin/append a tooltip window.
	@(link_name="ImGui_EndTooltip") EndTooltip :: proc "c" () --- // only call EndTooltip() if BeginTooltip()/BeginItemTooltip() returns true!
	@(link_name="ImGui_SetTooltip") SetTooltip :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
	...
	@(link_name="ImGui_BeginItemTooltip") BeginItemTooltip :: proc "c" () -> c.bool --- // begin/append a tooltip window if preceding item was hovered.
	@(link_name="ImGui_SetItemTooltip") SetItemTooltip :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // set a text-only tooltip if preceeding item was hovered. override any previous call to SetTooltip().
	// @(link_name="ImGui_SetItemTooltipV") SetItemTooltipV :: proc "c" (fmt: cstring, args: libc.va_list) ---
	// Popups: begin/end functions
	//  - BeginPopup(): query popup state, if open start appending into the window. Call EndPopup() afterwards. ImGuiWindowFlags are forwarded to the window.
	//  - BeginPopupModal(): block every interaction behind the window, cannot be closed by user, add a dimming background, has a title bar.
	@(link_name="ImGui_BeginPopup") BeginPopup :: proc "c" (str_id: cstring, flags: WindowFlags) -> c.bool --- // return true if the popup is open, and you can start outputting to it.
	@(link_name="ImGui_BeginPopupModal") BeginPopupModal :: proc "c" (name: cstring, p_open: ^c.bool, flags: WindowFlags) -> c.bool --- // return true if the modal is open, and you can start outputting to it.
	@(link_name="ImGui_EndPopup") EndPopup :: proc "c" () --- // only call EndPopup() if BeginPopupXXX() returns true!
	...
	@(link_name="ImGui_OpenPopup") OpenPopup :: proc "c" (str_id: cstring, popup_flags: PopupFlags) --- // call to mark popup as open (don't call every frame!).
	@(link_name="ImGui_OpenPopupID") OpenPopupID :: proc "c" (id: ID, popup_flags: PopupFlags) --- // id overload to facilitate calling from nested stacks
	@(link_name="ImGui_OpenPopupOnItemClick") OpenPopupOnItemClick :: proc "c" (str_id: cstring, popup_flags: PopupFlags) --- // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
	@(link_name="ImGui_CloseCurrentPopup") CloseCurrentPopup :: proc "c" () --- // manually close the popup we have begin-ed into.
	...
	@(link_name="ImGui_BeginPopupContextItem") BeginPopupContextItem :: proc "c" () -> c.bool --- // Implied str_id = NULL, popup_flags = 1
	@(link_name="ImGui_BeginPopupContextItemEx") BeginPopupContextItemEx :: proc "c" (str_id: cstring, popup_flags: PopupFlags) -> c.bool --- // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
	@(link_name="ImGui_BeginPopupContextWindow") BeginPopupContextWindow :: proc "c" () -> c.bool --- // Implied str_id = NULL, popup_flags = 1
	@(link_name="ImGui_BeginPopupContextWindowEx") BeginPopupContextWindowEx :: proc "c" (str_id: cstring, popup_flags: PopupFlags) -> c.bool --- // open+begin popup when clicked on current window.
	@(link_name="ImGui_BeginPopupContextVoid") BeginPopupContextVoid :: proc "c" () -> c.bool --- // Implied str_id = NULL, popup_flags = 1
	@(link_name="ImGui_BeginPopupContextVoidEx") BeginPopupContextVoidEx :: proc "c" (str_id: cstring, popup_flags: PopupFlags) -> c.bool --- // open+begin popup when clicked in void (where there are no windows).
	// Popups: query functions
	//  - IsPopupOpen(): return true if the popup is open at the current BeginPopup() level of the popup stack.
	//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup is open at the current BeginPopup() level of the popup stack.
	//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId + ImGuiPopupFlags_AnyPopupLevel: return true if any popup is open.
	@(link_name="ImGui_IsPopupOpen") IsPopupOpen :: proc "c" (str_id: cstring, flags: PopupFlags) -> c.bool --- // return true if the popup is open.
	// Tables
	...
	//        --------------------------------------------------------------------------------------------------------
	// - 5. Call EndTable()
	@(link_name="ImGui_BeginTable") BeginTable :: proc "c" (str_id: cstring, column: c.int, flags: TableFlags) -> c.bool --- // Implied outer_size = ImVec2(0.0f, 0.0f), inner_width = 0.0f
	@(link_name="ImGui_BeginTableEx") BeginTableEx :: proc "c" (str_id: cstring, column: c.int, flags: TableFlags, outer_size: Vec2, inner_width: f32) -> c.bool ---
	@(link_name="ImGui_EndTable") EndTable :: proc "c" () --- // only call EndTable() if BeginTable() returns true!
	@(link_name="ImGui_TableNextRow") TableNextRow :: proc "c" () --- // Implied row_flags = 0, min_row_height = 0.0f
	@(link_name="ImGui_TableNextRowEx") TableNextRowEx :: proc "c" (row_flags: TableRowFlags, min_row_height: f32) --- // append into the first cell of a new row.
	@(link_name="ImGui_TableNextColumn") TableNextColumn :: proc "c" () -> c.bool --- // append into the next column (or first column of next row if currently in last column). Return true when column is visible.
	@(link_name="ImGui_TableSetColumnIndex") TableSetColumnIndex :: proc "c" (column_n: c.int) -> c.bool --- // append into the specified column. Return true when column is visible.
	...
	@(link_name="ImGui_TableSetupColumn") TableSetupColumn :: proc "c" (label: cstring, flags: TableColumnFlags) --- // Implied init_width_or_weight = 0.0f, user_id = 0
	...
	@(link_name="ImGui_TableSetupScrollFreeze") TableSetupScrollFreeze :: proc "c" (cols: c.int, rows: c.int) --- // lock columns/rows so they stay visible when scrolled.
	@(link_name="ImGui_TableHeadersRow") TableHeadersRow :: proc "c" () --- // submit all headers cells based on data provided to TableSetupColumn() + submit context menu
	@(link_name="ImGui_TableHeader") TableHeader :: proc "c" (label: cstring) --- // submit one header cell manually (rarely used)
	...
	@(link_name="ImGui_TableGetSortSpecs") TableGetSortSpecs :: proc "c" () -> ^TableSortSpecs --- // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable().
	@(link_name="ImGui_TableGetColumnCount") TableGetColumnCount :: proc "c" () -> c.int --- // return number of columns (value passed to BeginTable)
	@(link_name="ImGui_TableGetColumnIndex") TableGetColumnIndex :: proc "c" () -> c.int --- // return current column index.
	@(link_name="ImGui_TableGetRowIndex") TableGetRowIndex :: proc "c" () -> c.int --- // return current row index.
	...
	@(link_name="ImGui_ColumnsEx") ColumnsEx :: proc "c" (count: c.int, id: cstring, border: c.bool) ---
	@(link_name="ImGui_NextColumn") NextColumn :: proc "c" () --- // next column, defaults to current row or next row if the current row is finished
	@(link_name="ImGui_GetColumnIndex") GetColumnIndex :: proc "c" () -> c.int --- // get current column index
	@(link_name="ImGui_GetColumnWidth") GetColumnWidth :: proc "c" (column_index: c.int) -> f32 --- // get column width (in pixels). pass -1 to use current column
	@(link_name="ImGui_SetColumnWidth") SetColumnWidth :: proc "c" (column_index: c.int, width: f32) --- // set column width (in pixels). pass -1 to use current column
	@(link_name="ImGui_GetColumnOffset") GetColumnOffset :: proc "c" (column_index: c.int) -> f32 --- // get position of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
	@(link_name="ImGui_SetColumnOffset") SetColumnOffset :: proc "c" (column_index: c.int, offset_x: f32) --- // set position of column line (in pixels, from the left side of the contents region). pass -1 to use current column
	@(link_name="ImGui_GetColumnsCount") GetColumnsCount :: proc "c" () -> c.int ---
	// Tab Bars, Tabs
	// - Note: Tabs are automatically created by the docking system (when in 'docking' branch). Use this to create tab bars/tabs yourself.
	@(link_name="ImGui_BeginTabBar") BeginTabBar :: proc "c" (str_id: cstring, flags: TabBarFlags) -> c.bool --- // create and append into a TabBar
	@(link_name="ImGui_EndTabBar") EndTabBar :: proc "c" () --- // only call EndTabBar() if BeginTabBar() returns true!
	@(link_name="ImGui_BeginTabItem") BeginTabItem :: proc "c" (label: cstring, p_open: ^c.bool, flags: TabItemFlags) -> c.bool --- // create a Tab. Returns true if the Tab is selected.
	@(link_name="ImGui_EndTabItem") EndTabItem :: proc "c" () --- // only call EndTabItem() if BeginTabItem() returns true!
	@(link_name="ImGui_TabItemButton") TabItemButton :: proc "c" (label: cstring, flags: TabItemFlags) -> c.bool --- // create a Tab behaving like a button. return true when clicked. cannot be selected in the tab bar.
	@(link_name="ImGui_SetTabItemClosed") SetTabItemClosed :: proc "c" (tab_or_docked_window_label: cstring) --- // notify TabBar or Docking system of a closed tab/window ahead (useful to reduce visual flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window name.
	// Docking
	...
	@(link_name="ImGui_DockSpace") DockSpace :: proc "c" (id: ID) -> ID --- // Implied size = ImVec2(0, 0), flags = 0, window_class = NULL
	@(link_name="ImGui_DockSpaceEx") DockSpaceEx :: proc "c" (id: ID, size: Vec2, flags: DockNodeFlags, window_class: ^WindowClass) -> ID ---
	@(link_name="ImGui_DockSpaceOverViewport") DockSpaceOverViewport :: proc "c" () -> ID --- // Implied viewport = NULL, flags = 0, window_class = NULL
	...
	@(link_name="ImGui_SetNextWindowDockID") SetNextWindowDockID :: proc "c" (dock_id: ID, cond: Cond) --- // set next window dock id
	@(link_name="ImGui_SetNextWindowClass") SetNextWindowClass :: proc "c" (window_class: ^WindowClass) --- // set next window class (control docking compatibility + provide hints to platform backend via custom viewport flags and platform parent/child relationship)
	@(link_name="ImGui_GetWindowDockID") GetWindowDockID :: proc "c" () -> ID ---
	@(link_name="ImGui_IsWindowDocked") IsWindowDocked :: proc "c" () -> c.bool --- // is current window docked into another window?
	// Logging/Capture
	// - All text output from the interface can be captured into tty/file/clipboard. By default, tree nodes are automatically opened during logging.
	@(link_name="ImGui_LogToTTY") LogToTTY :: proc "c" (auto_open_depth: c.int) --- // start logging to tty (stdout)
	@(link_name="ImGui_LogToFile") LogToFile :: proc "c" (auto_open_depth: c.int, filename: cstring) --- // start logging to file
	@(link_name="ImGui_LogToClipboard") LogToClipboard :: proc "c" (auto_open_depth: c.int) --- // start logging to OS clipboard
	@(link_name="ImGui_LogFinish") LogFinish :: proc "c" () --- // stop logging (close file, etc.)
	@(link_name="ImGui_LogButtons") LogButtons :: proc "c" () --- // helper to display buttons for logging to tty/file/clipboard
	@(link_name="ImGui_LogText") LogText :: proc "c" (fmt: cstring, #c_vararg args: ..any) --- // pass text data straight to log (without being displayed)
	...
	@(link_name="ImGui_BeginDragDropSource") BeginDragDropSource :: proc "c" (flags: DragDropFlags) -> c.bool --- // call after submitting an item which may be dragged. when this return true, you can call SetDragDropPayload() + EndDragDropSource()
	@(link_name="ImGui_SetDragDropPayload") SetDragDropPayload :: proc "c" (type: cstring, data: rawptr, sz: c.size_t, cond: Cond) -> c.bool --- // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
	@(link_name="ImGui_EndDragDropSource") EndDragDropSource :: proc "c" () --- // only call EndDragDropSource() if BeginDragDropSource() returns true!
	@(link_name="ImGui_BeginDragDropTarget") BeginDragDropTarget :: proc "c" () -> c.bool --- // call after submitting an item that may receive a payload. If this returns true, you can call AcceptDragDropPayload() + EndDragDropTarget()
	@(link_name="ImGui_AcceptDragDropPayload") AcceptDragDropPayload :: proc "c" (type: cstring, flags: DragDropFlags) -> ^Payload --- // accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the payload before the mouse button is released.
	@(link_name="ImGui_EndDragDropTarget") EndDragDropTarget :: proc "c" () --- // only call EndDragDropTarget() if BeginDragDropTarget() returns true!
	@(link_name="ImGui_GetDragDropPayload") GetDragDropPayload :: proc "c" () -> ^Payload --- // peek directly into the current payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to test for the payload type.
	// Disabling [BETA API]
	// - Disable all user interactions and dim items visuals (applying style.DisabledAlpha over current colors)
	// - Those can be nested but it cannot be used to enable an already disabled section (a single BeginDisabled(true) in the stack is enough to keep everything disabled)
	// - BeginDisabled(false) essentially does nothing useful but is provided to facilitate use of boolean expressions. If you can avoid calling BeginDisabled(False)/EndDisabled() best to avoid it.
	@(link_name="ImGui_BeginDisabled") BeginDisabled :: proc "c" (disabled: c.bool) ---
	@(link_name="ImGui_EndDisabled") EndDisabled :: proc "c" () ---
	// Clipping
	// - Mouse hovering is affected by ImGui::PushClipRect() calls, unlike direct calls to ImDrawList::PushClipRect() which are render only.
	@(link_name="ImGui_PushClipRect") PushClipRect :: proc "c" (clip_rect_min: Vec2, clip_rect_max: Vec2, intersect_with_current_clip_rect: c.bool) ---
	@(link_name="ImGui_PopClipRect") PopClipRect :: proc "c" () ---
	// Focus, Activation
	// - Prefer using "SetItemDefaultFocus()" over "if (IsWindowAppearing()) SetScrollHereY()" when applicable to signify "this is the default item"
	@(link_name="ImGui_SetItemDefaultFocus") SetItemDefaultFocus :: proc "c" () --- // make last item the default focused item of a window.
	@(link_name="ImGui_SetKeyboardFocusHere") SetKeyboardFocusHere :: proc "c" () --- // Implied offset = 0
	@(link_name="ImGui_SetKeyboardFocusHereEx") SetKeyboardFocusHereEx :: proc "c" (offset: c.int) --- // focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
	// Overlapping mode
	@(link_name="ImGui_SetNextItemAllowOverlap") SetNextItemAllowOverlap :: proc "c" () --- // allow next item to be overlapped by a subsequent item. Useful with invisible buttons, selectable, treenode covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.
	// Item/Widgets Utilities and Query Functions
	// - Most of the functions are referring to the previous Item that has been submitted.
	// - See Demo Window under "Widgets->Querying Status" for an interactive visualization of most of those functions.
	@(link_name="ImGui_IsItemHovered") IsItemHovered :: proc "c" (flags: HoveredFlags) -> c.bool --- // is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.
	@(link_name="ImGui_IsItemActive") IsItemActive :: proc "c" () -> c.bool --- // is the last item active? (e.g. button being held, text field being edited. This will continuously return true while holding mouse button on an item. Items that don't interact will always return false)
	@(link_name="ImGui_IsItemFocused") IsItemFocused :: proc "c" () -> c.bool --- // is the last item focused for keyboard/gamepad navigation?
	@(link_name="ImGui_IsItemClicked") IsItemClicked :: proc "c" () -> c.bool --- // Implied mouse_button = 0
	@(link_name="ImGui_IsItemClickedEx") IsItemClickedEx :: proc "c" (mouse_button: MouseButton) -> c.bool --- // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
	@(link_name="ImGui_IsItemVisible") IsItemVisible :: proc "c" () -> c.bool --- // is the last item visible? (items may be out of sight because of clipping/scrolling)
	@(link_name="ImGui_IsItemEdited") IsItemEdited :: proc "c" () -> c.bool --- // did the last item modify its underlying value this frame? or was pressed? This is generally the same as the "bool" return value of many widgets.
	@(link_name="ImGui_IsItemActivated") IsItemActivated :: proc "c" () -> c.bool --- // was the last item just made active (item was previously inactive).
	@(link_name="ImGui_IsItemDeactivated") IsItemDeactivated :: proc "c" () -> c.bool --- // was the last item just made inactive (item was previously active). Useful for Undo/Redo patterns with widgets that require continuous editing.
	@(link_name="ImGui_IsItemDeactivatedAfterEdit") IsItemDeactivatedAfterEdit :: proc "c" () -> c.bool --- // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag moved). Useful for Undo/Redo patterns with widgets that require continuous editing. Note that you may get false positives (some widgets such as Combo()/ListBox()/Selectable() will return true even when clicking an already selected item).
	@(link_name="ImGui_IsItemToggledOpen") IsItemToggledOpen :: proc "c" () -> c.bool --- // was the last item open state toggled? set by TreeNode().
	@(link_name="ImGui_IsAnyItemHovered") IsAnyItemHovered :: proc "c" () -> c.bool --- // is any item hovered?
	@(link_name="ImGui_IsAnyItemActive") IsAnyItemActive :: proc "c" () -> c.bool --- // is any item active?
	@(link_name="ImGui_IsAnyItemFocused") IsAnyItemFocused :: proc "c" () -> c.bool --- // is any item focused?
	@(link_name="ImGui_GetItemID") GetItemID :: proc "c" () -> ID --- // get ID of last item (~~ often same ImGui::GetID(label) beforehand)
	@(link_name="ImGui_GetItemRectMin") GetItemRectMin :: proc "c" () -> Vec2 --- // get upper-left bounding rectangle of the last item (screen space)
	@(link_name="ImGui_GetItemRectMax") GetItemRectMax :: proc "c" () -> Vec2 --- // get lower-right bounding rectangle of the last item (screen space)
	@(link_name="ImGui_GetItemRectSize") GetItemRectSize :: proc "c" () -> Vec2 --- // get size of last item
	// Viewports
	// - Currently represents the Platform Window created by the application which is hosting our Dear ImGui windows.
	// - In 'docking' branch with multi-viewport enabled, we extend this concept to have multiple active viewports.
	// - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform window" operation mode.
	@(link_name="ImGui_GetMainViewport") GetMainViewport :: proc "c" () -> ^Viewport --- // return primary/default viewport. This can never be NULL.
	// Background/Foreground Draw Lists
	@(link_name="ImGui_GetBackgroundDrawList") GetBackgroundDrawList :: proc "c" () -> ^DrawList --- // get background draw list for the viewport associated to the current window. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
	@(link_name="ImGui_GetForegroundDrawList") GetForegroundDrawList :: proc "c" () -> ^DrawList --- // get foreground draw list for the viewport associated to the current window. this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.
	@(link_name="ImGui_GetBackgroundDrawListImGuiViewportPtr") GetBackgroundDrawListImGuiViewportPtr :: proc "c" (viewport: ^Viewport) -> ^DrawList --- // get background draw list for the given viewport. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
	@(link_name="ImGui_GetForegroundDrawListImGuiViewportPtr") GetForegroundDrawListImGuiViewportPtr :: proc "c" (viewport: ^Viewport) -> ^DrawList --- // get foreground draw list for the given viewport. this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.
	// Miscellaneous Utilities
	@(link_name="ImGui_IsRectVisibleBySize") IsRectVisibleBySize :: proc "c" (size: Vec2) -> c.bool --- // test if rectangle (of given size, starting from cursor position) is visible / not clipped.
	@(link_name="ImGui_IsRectVisible") IsRectVisible :: proc "c" (rect_min: Vec2, rect_max: Vec2) -> c.bool --- // test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
	@(link_name="ImGui_GetTime") GetTime :: proc "c" () -> f64 --- // get global imgui time. incremented by io.DeltaTime every frame.
	@(link_name="ImGui_GetFrameCount") GetFrameCount :: proc "c" () -> c.int --- // get global imgui frame count. incremented by 1 every frame.
	@(link_name="ImGui_GetDrawListSharedData") GetDrawListSharedData :: proc "c" () -> ^DrawListSharedData --- // you may use this when creating your own ImDrawList instances.
	@(link_name="ImGui_GetStyleColorName") GetStyleColorName :: proc "c" (idx: Col) -> cstring --- // get a string corresponding to the enum value (for display, saving, etc.).
	@(link_name="ImGui_BeginChildFrame") BeginChildFrame :: proc "c" (id: ID, size: Vec2, flags: WindowFlags) -> c.bool --- // helper to create a child window / scrolling region that looks like a normal widget frame
	@(link_name="ImGui_EndChildFrame") EndChildFrame :: proc "c" () --- // always call EndChildFrame() regardless of BeginChildFrame() return values (which indicates a collapsed/clipped window)
	// Text Utilities
	...
	@(link_name="ImGui_IsKeyDown") IsKeyDown :: proc "c" (key: Key) -> c.bool --- // is key being held.
	@(link_name="ImGui_IsKeyPressed") IsKeyPressed :: proc "c" (key: Key) -> c.bool --- // Implied repeat = true
	@(link_name="ImGui_IsKeyPressedEx") IsKeyPressedEx :: proc "c" (key: Key, repeat: c.bool) -> c.bool --- // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
	@(link_name="ImGui_IsKeyReleased") IsKeyReleased :: proc "c" (key: Key) -> c.bool --- // was key released (went from Down to !Down)?
	@(link_name="ImGui_GetKeyPressedAmount") GetKeyPressedAmount :: proc "c" (key: Key, repeat_delay: f32, rate: f32) -> c.int --- // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
	@(link_name="ImGui_GetKeyName") GetKeyName :: proc "c" (key: Key) -> cstring --- // [DEBUG] returns English name of the key. Those names a provided for debugging purpose and are not meant to be saved persistently not compared.
	@(link_name="ImGui_SetNextFrameWantCaptureKeyboard") SetNextFrameWantCaptureKeyboard :: proc "c" (want_capture_keyboard: c.bool) --- // Override io.WantCaptureKeyboard flag next frame (said flag is left for your application to handle, typically when true it instructs your app to ignore inputs). e.g. force capture keyboard when your widget is being hovered. This is equivalent to setting "io.WantCaptureKeyboard = want_capture_keyboard"; after the next NewFrame() call.
	// Inputs Utilities: Mouse specific
	// - To refer to a mouse button, you may use named enums in your code e.g. ImGuiMouseButton_Left, ImGuiMouseButton_Right.
	// - You can also use regular integer: it is forever guaranteed that 0=Left, 1=Right, 2=Middle.
	// - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking position (see 'lock_threshold' and 'io.MouseDraggingThreshold')
	@(link_name="ImGui_IsMouseDown") IsMouseDown :: proc "c" (button: MouseButton) -> c.bool --- // is mouse button held?
	@(link_name="ImGui_IsMouseClicked") IsMouseClicked :: proc "c" (button: MouseButton) -> c.bool --- // Implied repeat = false
	@(link_name="ImGui_IsMouseClickedEx") IsMouseClickedEx :: proc "c" (button: MouseButton, repeat: c.bool) -> c.bool --- // did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
	@(link_name="ImGui_IsMouseReleased") IsMouseReleased :: proc "c" (button: MouseButton) -> c.bool --- // did mouse button released? (went from Down to !Down)
	@(link_name="ImGui_IsMouseDoubleClicked") IsMouseDoubleClicked :: proc "c" (button: MouseButton) -> c.bool --- // did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note that a double-click will also report IsMouseClicked() == true)
	@(link_name="ImGui_GetMouseClickedCount") GetMouseClickedCount :: proc "c" (button: MouseButton) -> c.int --- // return the number of successive mouse-clicks at the time where a click happen (otherwise 0).
	@(link_name="ImGui_IsMouseHoveringRect") IsMouseHoveringRect :: proc "c" (r_min: Vec2, r_max: Vec2) -> c.bool --- // Implied clip = true
	@(link_name="ImGui_IsMouseHoveringRectEx") IsMouseHoveringRectEx :: proc "c" (r_min: Vec2, r_max: Vec2, clip: c.bool) -> c.bool --- // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
	@(link_name="ImGui_IsMousePosValid") IsMousePosValid :: proc "c" (mouse_pos: ^Vec2) -> c.bool --- // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
	@(link_name="ImGui_IsAnyMouseDown") IsAnyMouseDown :: proc "c" () -> c.bool --- // [WILL OBSOLETE] is any mouse button held? This was designed for backends, but prefer having backend maintain a mask of held mouse buttons, because upcoming input queue system will make this invalid.
	@(link_name="ImGui_GetMousePos") GetMousePos :: proc "c" () -> Vec2 --- // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
	@(link_name="ImGui_GetMousePosOnOpeningCurrentPopup") GetMousePosOnOpeningCurrentPopup :: proc "c" () -> Vec2 --- // retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
	@(link_name="ImGui_IsMouseDragging") IsMouseDragging :: proc "c" (button: MouseButton, lock_threshold: f32) -> c.bool --- // is mouse dragging? (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
	@(link_name="ImGui_GetMouseDragDelta") GetMouseDragDelta :: proc "c" (button: MouseButton, lock_threshold: f32) -> Vec2 --- // return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
	@(link_name="ImGui_ResetMouseDragDelta") ResetMouseDragDelta :: proc "c" () --- // Implied button = 0
	@(link_name="ImGui_ResetMouseDragDeltaEx") ResetMouseDragDeltaEx :: proc "c" (button: MouseButton) --- //
	@(link_name="ImGui_GetMouseCursor") GetMouseCursor :: proc "c" () -> MouseCursor --- // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
	@(link_name="ImGui_SetMouseCursor") SetMouseCursor :: proc "c" (cursor_type: MouseCursor) --- // set desired mouse cursor shape
	@(link_name="ImGui_SetNextFrameWantCaptureMouse") SetNextFrameWantCaptureMouse :: proc "c" (want_capture_mouse: c.bool) --- // Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instucts your app to ignore inputs). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.
	// Clipboard Utilities
	...
	@(link_name="ImGui_LoadIniSettingsFromDisk") LoadIniSettingsFromDisk :: proc "c" (ini_filename: cstring) --- // call after CreateContext() and before the first call to NewFrame(). NewFrame() automatically calls LoadIniSettingsFromDisk(io.IniFilename).
	@(link_name="ImGui_LoadIniSettingsFromMemory") LoadIniSettingsFromMemory :: proc "c" (ini_data: cstring, ini_size: c.size_t) --- // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
	@(link_name="ImGui_SaveIniSettingsToDisk") SaveIniSettingsToDisk :: proc "c" (ini_filename: cstring) --- // this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .ini file (and also by DestroyContext).
	@(link_name="ImGui_SaveIniSettingsToMemory") SaveIniSettingsToMemory :: proc "c" (out_ini_size: ^c.size_t) -> cstring --- // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
	// Debug Utilities
	@(link_name="ImGui_DebugTextEncoding") DebugTextEncoding :: proc "c" (text: cstring) ---
	@(link_name="ImGui_DebugCheckVersionAndDataLayout") DebugCheckVersionAndDataLayout :: proc "c" (version_str: cstring, sz_io: c.size_t, sz_style: c.size_t, sz_vec2: c.size_t, sz_vec4: c.size_t, sz_drawvert: c.size_t, sz_drawidx: c.size_t) -> c.bool --- // This is called by IMGUI_CHECKVERSION() macro.
	// Memory Allocators
	...
	@(link_name="ImGui_GetPlatformIO") GetPlatformIO :: proc "c" () -> ^PlatformIO --- // platform/renderer functions, for backend to setup + viewports list.
	@(link_name="ImGui_UpdatePlatformWindows") UpdatePlatformWindows :: proc "c" () --- // call in main loop. will call CreateWindow/ResizeWindow/etc. platform functions for each secondary viewport, and DestroyWindow for each inactive viewport.
	@(link_name="ImGui_RenderPlatformWindowsDefault") RenderPlatformWindowsDefault :: proc "c" () --- // Implied platform_render_arg = NULL, renderer_render_arg = NULL
	@(link_name="ImGui_RenderPlatformWindowsDefaultEx") RenderPlatformWindowsDefaultEx :: proc "c" (platform_render_arg: rawptr, renderer_render_arg: rawptr) --- // call in main loop. will call RenderWindow/SwapBuffers platform functions for each secondary viewport which doesn't have the ImGuiViewportFlags_Minimized flag set. May be reimplemented by user for custom rendering needs.
	@(link_name="ImGui_DestroyPlatformWindows") DestroyPlatformWindows :: proc "c" () --- // call DestroyWindow platform functions for all viewports. call from backend Shutdown() if you need to close platform windows before imgui shutdown. otherwise will be called by DestroyContext().
	@(link_name="ImGui_FindViewportByID") FindViewportByID :: proc "c" (id: ID) -> ^Viewport --- // this is a helper for backends.
	@(link_name="ImGui_FindViewportByPlatformHandle") FindViewportByPlatformHandle :: proc "c" (platform_handle: rawptr) -> ^Viewport --- // this is a helper for backends. the type platform_handle is decided by the backend (e.g. HWND, MyWindow*, GLFWwindow* etc.)
	@(link_name="ImVector_Construct") Vector_Construct :: proc "c" (vector: rawptr) --- // Construct a zero-size ImVector<> (of any type). This is primarily useful when calling ImFontGlyphRangesBuilder_BuildRanges()
	@(link_name="ImVector_Destruct") Vector_Destruct :: proc "c" (vector: rawptr) --- // Destruct an ImVector<> (of any type). Important: Frees the vector memory but does not call destructors on contained objects (if they have them)
	@(link_name="ImGuiStyle_ScaleAllSizes") Style_ScaleAllSizes :: proc "c" (self: ^Style, scale_factor: f32) ---
	// Input Functions
	@(link_name="ImGuiIO_AddKeyEvent") IO_AddKeyEvent :: proc "c" (self: ^IO, key: Key, down: c.bool) --- // Queue a new key down/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
	@(link_name="ImGuiIO_AddKeyAnalogEvent") IO_AddKeyAnalogEvent :: proc "c" (self: ^IO, key: Key, down: c.bool, v: f32) --- // Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
	@(link_name="ImGuiIO_AddMousePosEvent") IO_AddMousePosEvent :: proc "c" (self: ^IO, x: f32, y: f32) --- // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)
	@(link_name="ImGuiIO_AddMouseButtonEvent") IO_AddMouseButtonEvent :: proc "c" (self: ^IO, button: c.int, down: c.bool) --- // Queue a mouse button change
	@(link_name="ImGuiIO_AddMouseWheelEvent") IO_AddMouseWheelEvent :: proc "c" (self: ^IO, wheel_x: f32, wheel_y: f32) --- // Queue a mouse wheel update. wheel_y<0: scroll down, wheel_y>0: scroll up, wheel_x<0: scroll right, wheel_x>0: scroll left.
	...
	@(link_name="ImGuiIO_SetAppAcceptingEvents") IO_SetAppAcceptingEvents :: proc "c" (self: ^IO, accepting_events: c.bool) --- // Set master flag for accepting key/mouse/text events (default to true). Useful if you have native dialog boxes that are interrupting your application loop/refresh, and you want to disable events being queued while your app is frozen.
	@(link_name="ImGuiIO_ClearEventsQueue") IO_ClearEventsQueue :: proc "c" (self: ^IO) --- // Clear all incoming events.
	@(link_name="ImGuiIO_ClearInputKeys") IO_ClearInputKeys :: proc "c" (self: ^IO) --- // Clear current keyboard/mouse/gamepad state + current frame text input buffer. Equivalent to releasing all keys/buttons.
	@(link_name="ImGuiIO_ClearInputCharacters") IO_ClearInputCharacters :: proc "c" (self: ^IO) --- // [Obsolete] Clear the current frame text input buffer. Now included within ClearInputKeys().
	...
	@(link_name="ImGuiInputTextCallbackData_HasSelection") InputTextCallbackData_HasSelection :: proc "c" (self: ^InputTextCallbackData) -> c.bool ---
	@(link_name="ImGuiPayload_Clear") Payload_Clear :: proc "c" (self: ^Payload) ---
	@(link_name="ImGuiPayload_IsDataType") Payload_IsDataType :: proc "c" (self: ^Payload, type: cstring) -> c.bool ---
	@(link_name="ImGuiPayload_IsPreview") Payload_IsPreview :: proc "c" (self: ^Payload) -> c.bool ---
	@(link_name="ImGuiPayload_IsDelivery") Payload_IsDelivery :: proc "c" (self: ^Payload) -> c.bool ---
	@(link_name="ImGuiTextFilter_ImGuiTextRange_empty") TextFilter_ImGuiTextRange_empty :: proc "c" (self: ^TextFilter_ImGuiTextRange) -> c.bool ---
	...
	@(link_name="ImGuiTextFilter_Draw") TextFilter_Draw :: proc "c" (self: ^TextFilter, label: cstring, width: f32) -> c.bool --- // Helper calling InputText+Build
	@(link_name="ImGuiTextFilter_PassFilter") TextFilter_PassFilter :: proc "c" (self: ^TextFilter, text: cstring, text_end: cstring) -> c.bool ---
	...
	@(link_name="ImGuiTextFilter_IsActive") TextFilter_IsActive :: proc "c" (self: ^TextFilter) -> c.bool ---
	@(link_name="ImGuiTextBuffer_begin") TextBuffer_begin :: proc "c" (self: ^TextBuffer) -> cstring ---
	@(link_name="ImGuiTextBuffer_end") TextBuffer_end :: proc "c" (self: ^TextBuffer) -> cstring --- // Buf is zero-terminated, so end() will point on the zero-terminator
	@(link_name="ImGuiTextBuffer_size") TextBuffer_size :: proc "c" (self: ^TextBuffer) -> c.int ---
	@(link_name="ImGuiTextBuffer_empty") TextBuffer_empty :: proc "c" (self: ^TextBuffer) -> c.bool ---
	...
	@(link_name="ImGuiListClipper_End") ListClipper_End :: proc "c" (self: ^ListClipper) --- // Automatically called on the last call of Step() that returns false.
	@(link_name="ImGuiListClipper_Step") ListClipper_Step :: proc "c" (self: ^ListClipper) -> c.bool --- // Call until it returns false. The DisplayStart/DisplayEnd fields will be set and you can process/draw those items.
	// Call IncludeItemByIndex() or IncludeItemsByIndex() *BEFORE* first call to Step() if you need a range of items to not be clipped, regardless of their visibility.
	// (Due to alignment / padding of certain items it is possible that an extra item may be included on either end of the display range).
	@(link_name="ImGuiListClipper_IncludeItemByIndex") ListClipper_IncludeItemByIndex :: proc "c" (self: ^ListClipper, item_index: c.int) ---
	@(link_name="ImGuiListClipper_IncludeItemsByIndex") ListClipper_IncludeItemsByIndex :: proc "c" (self: ^ListClipper, item_begin: c.int, item_end: c.int) --- // item_end is exclusive e.g. use (42, 42+1) to make item 42 never clipped.
	@(link_name="ImGuiListClipper_IncludeRangeByIndices") ListClipper_IncludeRangeByIndices :: proc "c" (self: ^ListClipper, item_begin: c.int, item_end: c.int) --- // [renamed in 1.89.9]
	@(link_name="ImGuiListClipper_ForceDisplayRangeByIndices") ListClipper_ForceDisplayRangeByIndices :: proc "c" (self: ^ListClipper, item_begin: c.int, item_end: c.int) --- // [renamed in 1.89.6]
	...
	@(link_name="ImDrawListSplitter_Clear") DrawListSplitter_Clear :: proc "c" (self: ^DrawListSplitter) --- // Do not clear Channels[] so our allocations are reused next frame
	...
	@(link_name="ImDrawList_PushClipRect") DrawList_PushClipRect :: proc "c" (self: ^DrawList, clip_rect_min: Vec2, clip_rect_max: Vec2, intersect_with_current_clip_rect: c.bool) --- // Render-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
	...
	@(link_name="ImDrawList_PathEllipticalArcToEx") DrawList_PathEllipticalArcToEx :: proc "c" (self: ^DrawList, center: Vec2, radius_x: f32, radius_y: f32, rot: f32, a_min: f32, a_max: f32, num_segments: c.int) --- // Ellipse
	@(link_name="ImDrawList_PathBezierCubicCurveTo") DrawList_PathBezierCubicCurveTo :: proc "c" (self: ^DrawList, p2: Vec2, p3: Vec2, p4: Vec2, num_segments: c.int) --- // Cubic Bezier (4 control points)
	@(link_name="ImDrawList_PathBezierQuadraticCurveTo") DrawList_PathBezierQuadraticCurveTo :: proc "c" (self: ^DrawList, p2: Vec2, p3: Vec2, num_segments: c.int) --- // Quadratic Bezier (3 control points)
	...
	// Advanced
	@(link_name="ImDrawList_AddCallback") DrawList_AddCallback :: proc "c" (self: ^DrawList, callback: DrawCallback, callback_data: rawptr) --- // Your rendering function must check for 'UserCallback' in ImDrawCmd and call the function instead of rendering triangles.
	@(link_name="ImDrawList_AddDrawCmd") DrawList_AddDrawCmd :: proc "c" (self: ^DrawList) --- // This is useful if you need to forcefully create a new draw call (to allow for dependent rendering / blending). Otherwise primitives are merged into the same draw-call as much as possible
	@(link_name="ImDrawList_CloneOutput") DrawList_CloneOutput :: proc "c" (self: ^DrawList) -> ^DrawList --- // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer.
	// Advanced: Channels
	...
	@(link_name="ImDrawData_AddDrawList") DrawData_AddDrawList :: proc "c" (self: ^DrawData, draw_list: ^DrawList) --- // Helper to add an external draw list into an existing ImDrawData.
	@(link_name="ImDrawData_DeIndexAllBuffers") DrawData_DeIndexAllBuffers :: proc "c" (self: ^DrawData) --- // Helper to convert all buffers from indexed to non-indexed, in case you cannot render indexed. Note: this is slow and most likely a waste of resources. Always prefer indexed rendering!
	@(link_name="ImDrawData_ScaleClipRects") DrawData_ScaleClipRects :: proc "c" (self: ^DrawData, fb_scale: Vec2) --- // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
	...
	@(link_name="ImFontAtlasCustomRect_IsPacked") FontAtlasCustomRect_IsPacked :: proc "c" (self: ^FontAtlasCustomRect) -> c.bool ---
	...
	@(link_name="ImFontAtlas_ClearFonts") FontAtlas_ClearFonts :: proc "c" (self: ^FontAtlas) --- // Clear output font data (glyphs storage, UV coordinates).
	@(link_name="ImFontAtlas_Clear") FontAtlas_Clear :: proc "c" (self: ^FontAtlas) --- // Clear all input and output.
	// Build atlas, retrieve pixel data.
	// User is in charge of copying the pixels into graphics memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
	// The pitch is always = Width * BytesPerPixels (1 or 4)
	// Building in RGBA32 format is provided for convenience and compatibility, but note that unless you manually manipulate or copy color data into
	// the texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white (~75% of memory/bandwidth waste.
	@(link_name="ImFontAtlas_Build") FontAtlas_Build :: proc "c" (self: ^FontAtlas) -> c.bool --- // Build pixels data. This is called automatically for you by the GetTexData*** functions.
	@(link_name="ImFontAtlas_GetTexDataAsAlpha8") FontAtlas_GetTexDataAsAlpha8 :: proc "c" (self: ^FontAtlas, out_pixels: ^^c.uchar, out_width: ^c.int, out_height: ^c.int, out_bytes_per_pixel: ^c.int) --- // 1 byte per-pixel
	@(link_name="ImFontAtlas_GetTexDataAsRGBA32") FontAtlas_GetTexDataAsRGBA32 :: proc "c" (self: ^FontAtlas, out_pixels: ^^c.uchar, out_width: ^c.int, out_height: ^c.int, out_bytes_per_pixel: ^c.int) --- // 4 bytes-per-pixel
	@(link_name="ImFontAtlas_IsBuilt") FontAtlas_IsBuilt :: proc "c" (self: ^FontAtlas) -> c.bool --- // Bit ambiguous: used to detect when user didn't build texture but effectively we should check TexID != 0 except that would be backend dependent...
	...
	@(link_name="ImFontAtlas_GetGlyphRangesDefault") FontAtlas_GetGlyphRangesDefault :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Basic Latin, Extended Latin
	@(link_name="ImFontAtlas_GetGlyphRangesGreek") FontAtlas_GetGlyphRangesGreek :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Greek and Coptic
	@(link_name="ImFontAtlas_GetGlyphRangesKorean") FontAtlas_GetGlyphRangesKorean :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Korean characters
	@(link_name="ImFontAtlas_GetGlyphRangesJapanese") FontAtlas_GetGlyphRangesJapanese :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Hiragana, Katakana, Half-Width, Selection of 2999 Ideographs
	@(link_name="ImFontAtlas_GetGlyphRangesChineseFull") FontAtlas_GetGlyphRangesChineseFull :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Half-Width + Japanese Hiragana/Katakana + full set of about 21000 CJK Unified Ideographs
	@(link_name="ImFontAtlas_GetGlyphRangesChineseSimplifiedCommon") FontAtlas_GetGlyphRangesChineseSimplifiedCommon :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Half-Width + Japanese Hiragana/Katakana + set of 2500 CJK Unified Ideographs for common simplified Chinese
	@(link_name="ImFontAtlas_GetGlyphRangesCyrillic") FontAtlas_GetGlyphRangesCyrillic :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + about 400 Cyrillic characters
	@(link_name="ImFontAtlas_GetGlyphRangesThai") FontAtlas_GetGlyphRangesThai :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Thai characters
	@(link_name="ImFontAtlas_GetGlyphRangesVietnamese") FontAtlas_GetGlyphRangesVietnamese :: proc "c" (self: ^FontAtlas) -> ^Wchar --- // Default + Vietnamese characters
	...
	@(link_name="ImGui_ListBoxObsolete") ListBoxObsolete :: proc "c" (label: cstring, current_item: ^c.int, old_callback: proc "c" (user_data: rawptr, idx: c.int, out_text: ^cstring) -> c.bool, user_data: rawptr, items_count: c.int) -> c.bool --- // Implied height_in_items = -1
	@(link_name="ImGui_ListBoxObsoleteEx") ListBoxObsoleteEx :: proc "c" (label: cstring, current_item: ^c.int, old_callback: proc "c" (user_data: rawptr, idx: c.int, out_text: ^cstring) -> c.bool, user_data: rawptr, items_count: c.int, height_in_items: c.int) -> c.bool ---
	@(link_name="ImGui_ComboObsolete") ComboObsolete :: proc "c" (label: cstring, current_item: ^c.int, old_callback: proc "c" (user_data: rawptr, idx: c.int, out_text: ^cstring) -> c.bool, user_data: rawptr, items_count: c.int) -> c.bool --- // Implied popup_max_height_in_items = -1
	@(link_name="ImGui_ComboObsoleteEx") ComboObsoleteEx :: proc "c" (label: cstring, current_item: ^c.int, old_callback: proc "c" (user_data: rawptr, idx: c.int, out_text: ^cstring) -> c.bool, user_data: rawptr, items_count: c.int, popup_max_height_in_items: c.int) -> c.bool ---
	// OBSOLETED in 1.89.7 (from June 2023)
	@(link_name="ImGui_SetItemAllowOverlap") SetItemAllowOverlap :: proc "c" () --- // Use SetNextItemAllowOverlap() before item.
	// OBSOLETED in 1.89.4 (from March 2023)
	@(link_name="ImGui_PushAllowKeyboardFocus") PushAllowKeyboardFocus :: proc "c" (tab_stop: c.bool) ---
	@(link_name="ImGui_PopAllowKeyboardFocus") PopAllowKeyboardFocus :: proc "c" () ---
	// OBSOLETED in 1.89 (from August 2022)
	@(link_name="ImGui_ImageButtonImTextureID") ImageButtonImTextureID :: proc "c" (user_texture_id: TextureID, size: Vec2, uv0: Vec2, uv1: Vec2, frame_padding: c.int, bg_col: Vec4, tint_col: Vec4) -> c.bool --- // Use new ImageButton() signature (explicit item id, regular FramePadding)
	// OBSOLETED in 1.88 (from May 2022)
	@(link_name="ImGui_CaptureKeyboardFromApp") CaptureKeyboardFromApp :: proc "c" (want_capture_keyboard: c.bool) --- // Renamed as name was misleading + removed default value.
	@(link_name="ImGui_CaptureMouseFromApp") CaptureMouseFromApp :: proc "c" (want_capture_mouse: c.bool) --- // Renamed as name was misleading + removed default value.
	// OBSOLETED in 1.86 (from November 2021)
	@(link_name="ImGui_CalcListClipping") CalcListClipping :: proc "c" (items_count: c.int, items_height: f32, out_items_display_start: ^c.int, out_items_display_end: ^c.int) --- // Calculate coarse clipping for large list of evenly sized items. Prefer using ImGuiListClipper.
}
