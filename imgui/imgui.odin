...
foreign lib {
	...
	@(link_name="ImGui_ShowDemoWindow") ShowDemoWindow :: proc "c" (p_open: ^bool) --- // create Demo window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
	@(link_name="ImGui_ShowMetricsWindow") ShowMetricsWindow :: proc "c" (p_open: ^bool) --- // create Metrics/Debugger window. display Dear ImGui internals: windows, draw commands, various internal state, etc.
	@(link_name="ImGui_ShowDebugLogWindow") ShowDebugLogWindow :: proc "c" (p_open: ^bool) --- // create Debug Log window. display a simplified log of important dear imgui events.
	@(link_name="ImGui_ShowStackToolWindow") ShowStackToolWindow :: proc "c" (p_open: ^bool) --- // create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
	@(link_name="ImGui_ShowAboutWindow") ShowAboutWindow :: proc "c" (p_open: ^bool) --- // create About window. display Dear ImGui version, credits and build/system information.
	...
	@(link_name="ImGui_ShowStyleSelector") ShowStyleSelector :: proc "c" (label: cstring) -> bool --- // add style selector block (not a window), essentially a combo listing the default styles.
	...
	@(link_name="ImGui_IsWindowFocused") IsWindowFocused :: proc "c" (flags: FocusedFlags) -> bool --- // is current window focused? or its root/child, depending on flags. see flags for options.
	...
	@(link_name="ImGui_SetNextWindowCollapsed") SetNextWindowCollapsed :: proc "c" (collapsed: bool, cond: Cond) --- // set next window collapsed state. call before Begin()
	...
	@(link_name="ImGui_SetWindowCollapsed") SetWindowCollapsed :: proc "c" (collapsed: bool, cond: Cond) --- // (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
	...
	@(link_name="ImGui_SetWindowCollapsedStr") SetWindowCollapsedStr :: proc "c" (name: cstring, collapsed: bool, cond: Cond) --- // set named window collapsed state
	...
	@(link_name="ImGui_PushTabStop") PushTabStop :: proc "c" (tab_stop: bool) --- // == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default but you can disable it for certain widgets
	@(link_name="ImGui_PopTabStop") PopTabStop :: proc "c" () ---
	@(link_name="ImGui_PushButtonRepeat") PushButtonRepeat :: proc "c" (repeat: bool) --- // in 'repeat' mode, Button*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button() to tell if the button is held in the current frame.
	...
	@(link_name="ImGui_Button") Button :: proc "c" (label: cstring) -> bool --- // Implied size = ImVec2(0, 0)
	@(link_name="ImGui_ButtonEx") ButtonEx :: proc "c" (label: cstring, size: Vec2) -> bool --- // button
	@(link_name="ImGui_SmallButton") SmallButton :: proc "c" (label: cstring) -> bool --- // button with FramePadding=(0,0) to easily embed within text
	@(link_name="ImGui_InvisibleButton") InvisibleButton :: proc "c" (str_id: cstring, size: Vec2, flags: ButtonFlags) -> bool --- // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
	@(link_name="ImGui_ArrowButton") ArrowButton :: proc "c" (str_id: cstring, dir: Dir) -> bool --- // square button with an arrow shape
	...
	@(link_name="ImGui_RadioButton") RadioButton :: proc "c" (label: cstring, active: bool) -> bool --- // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
	@(link_name="ImGui_RadioButtonIntPtr") RadioButtonIntPtr :: proc "c" (label: cstring, v: ^c.int, v_button: c.int) -> bool --- // shortcut to handle the above pattern when value is an integer
	...
	@(link_name="ImGui_ImageButton") ImageButton :: proc "c" (str_id: cstring, user_texture_id: TextureID, size: Vec2) -> bool --- // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
	...
	@(link_name="ImGui_ComboChar") ComboChar :: proc "c" (label: cstring, current_item: ^c.int, items: [^]cstring, items_count: c.int) -> bool --- // Implied popup_max_height_in_items = -1
	...
	@(link_name="ImGui_DragFloat") DragFloat :: proc "c" (label: cstring, v: ^f32) -> bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	@(link_name="ImGui_DragFloatEx") DragFloatEx :: proc "c" (label: cstring, v: ^f32, v_speed: f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> bool --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragFloat2") DragFloat2 :: proc "c" (label: cstring, v: ^[2]f32) -> bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	...
	@(link_name="ImGui_DragFloat3") DragFloat3 :: proc "c" (label: cstring, v: ^[3]f32) -> bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	...
	@(link_name="ImGui_DragFloat4") DragFloat4 :: proc "c" (label: cstring, v: ^[4]f32) -> bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
	...
	@(link_name="ImGui_DragFloatRange2") DragFloatRange2 :: proc "c" (label: cstring, v_current_min: ^f32, v_current_max: ^f32) -> bool --- // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", format_max = NULL, flags = 0
	...
	@(link_name="ImGui_DragInt") DragInt :: proc "c" (label: cstring, v: ^c.int) -> bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	@(link_name="ImGui_DragIntEx") DragIntEx :: proc "c" (label: cstring, v: ^c.int, v_speed: f32, v_min: c.int, v_max: c.int, format: cstring, flags: SliderFlags) -> bool --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragInt2") DragInt2 :: proc "c" (label: cstring, v: ^[2]c.int) -> bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	...
	@(link_name="ImGui_DragInt3") DragInt3 :: proc "c" (label: cstring, v: ^[3]c.int) -> bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	...
	@(link_name="ImGui_DragInt4") DragInt4 :: proc "c" (label: cstring, v: ^[4]c.int) -> bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
	...
	@(link_name="ImGui_DragIntRange2") DragIntRange2 :: proc "c" (label: cstring, v_current_min: ^c.int, v_current_max: ^c.int) -> bool --- // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", format_max = NULL, flags = 0
	...
	@(link_name="ImGui_DragScalar") DragScalar :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr) -> bool --- // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
	...
	@(link_name="ImGui_DragScalarN") DragScalarN :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int) -> bool --- // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
	...
	//   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
	@(link_name="ImGui_SliderFloat") SliderFloat :: proc "c" (label: cstring, v: ^f32, v_min: f32, v_max: f32) -> bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_SliderFloatEx") SliderFloatEx :: proc "c" (label: cstring, v: ^f32, v_min: f32, v_max: f32, format: cstring, flags: SliderFlags) -> bool --- // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
	@(link_name="ImGui_SliderFloat2") SliderFloat2 :: proc "c" (label: cstring, v: ^[2]f32, v_min: f32, v_max: f32) -> bool --- // Implied format = "%.3f", flags = 0
	...
	@(link_name="ImGui_SliderFloat3") SliderFloat3 :: proc "c" (label: cstring, v: ^[3]f32, v_min: f32, v_max: f32) -> bool --- // Implied format = "%.3f", flags = 0
	...
	@(link_name="ImGui_SliderFloat4") SliderFloat4 :: proc "c" (label: cstring, v: ^[4]f32, v_min: f32, v_max: f32) -> bool --- // Implied format = "%.3f", flags = 0
	...
	@(link_name="ImGui_SliderAngle") SliderAngle :: proc "c" (label: cstring, v_rad: ^f32) -> bool --- // Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f, format = "%.0f deg", flags = 0
	...
	@(link_name="ImGui_SliderInt") SliderInt :: proc "c" (label: cstring, v: ^c.int, v_min: c.int, v_max: c.int) -> bool --- // Implied format = "%d", flags = 0
	...
	@(link_name="ImGui_SliderInt2") SliderInt2 :: proc "c" (label: cstring, v: ^[2]c.int, v_min: c.int, v_max: c.int) -> bool --- // Implied format = "%d", flags = 0
	...
	@(link_name="ImGui_SliderInt3") SliderInt3 :: proc "c" (label: cstring, v: ^[3]c.int, v_min: c.int, v_max: c.int) -> bool --- // Implied format = "%d", flags = 0
	...
	@(link_name="ImGui_SliderInt4") SliderInt4 :: proc "c" (label: cstring, v: ^[4]c.int, v_min: c.int, v_max: c.int) -> bool --- // Implied format = "%d", flags = 0
	...
	@(link_name="ImGui_SliderScalar") SliderScalar :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr) -> bool --- // Implied format = NULL, flags = 0
	...
	@(link_name="ImGui_SliderScalarN") SliderScalarN :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int, p_min: rawptr, p_max: rawptr) -> bool --- // Implied format = NULL, flags = 0
	...
	@(link_name="ImGui_VSliderFloat") VSliderFloat :: proc "c" (label: cstring, size: Vec2, v: ^f32, v_min: f32, v_max: f32) -> bool --- // Implied format = "%.3f", flags = 0
	...
	@(link_name="ImGui_VSliderInt") VSliderInt :: proc "c" (label: cstring, size: Vec2, v: ^c.int, v_min: c.int, v_max: c.int) -> bool --- // Implied format = "%d", flags = 0
	...
	@(link_name="ImGui_VSliderScalar") VSliderScalar :: proc "c" (label: cstring, size: Vec2, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr) -> bool --- // Implied format = NULL, flags = 0
	...
	@(link_name="ImGui_InputText") InputText :: proc "c" (label: cstring, buf: cstring, buf_size: c.size_t, flags: InputTextFlags) -> bool --- // Implied callback = NULL, user_data = NULL
	...
	@(link_name="ImGui_InputTextMultiline") InputTextMultiline :: proc "c" (label: cstring, buf: cstring, buf_size: c.size_t) -> bool --- // Implied size = ImVec2(0, 0), flags = 0, callback = NULL, user_data = NULL
	...
	@(link_name="ImGui_InputTextWithHint") InputTextWithHint :: proc "c" (label: cstring, hint: cstring, buf: cstring, buf_size: c.size_t, flags: InputTextFlags) -> bool --- // Implied callback = NULL, user_data = NULL
	...
	@(link_name="ImGui_InputFloat") InputFloat :: proc "c" (label: cstring, v: ^f32) -> bool --- // Implied step = 0.0f, step_fast = 0.0f, format = "%.3f", flags = 0
	...
	@(link_name="ImGui_InputFloat2") InputFloat2 :: proc "c" (label: cstring, v: ^[2]f32) -> bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloat2Ex") InputFloat2Ex :: proc "c" (label: cstring, v: ^[2]f32, format: cstring, flags: InputTextFlags) -> bool ---
	@(link_name="ImGui_InputFloat3") InputFloat3 :: proc "c" (label: cstring, v: ^[3]f32) -> bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloat3Ex") InputFloat3Ex :: proc "c" (label: cstring, v: ^[3]f32, format: cstring, flags: InputTextFlags) -> bool ---
	@(link_name="ImGui_InputFloat4") InputFloat4 :: proc "c" (label: cstring, v: ^[4]f32) -> bool --- // Implied format = "%.3f", flags = 0
	@(link_name="ImGui_InputFloat4Ex") InputFloat4Ex :: proc "c" (label: cstring, v: ^[4]f32, format: cstring, flags: InputTextFlags) -> bool ---
	@(link_name="ImGui_InputInt") InputInt :: proc "c" (label: cstring, v: ^c.int) -> bool --- // Implied step = 1, step_fast = 100, flags = 0
	...
	@(link_name="ImGui_InputDouble") InputDouble :: proc "c" (label: cstring, v: ^f64) -> bool --- // Implied step = 0.0, step_fast = 0.0, format = "%.6f", flags = 0
	...
	@(link_name="ImGui_InputScalar") InputScalar :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr) -> bool --- // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
	...
	@(link_name="ImGui_InputScalarN") InputScalarN :: proc "c" (label: cstring, data_type: DataType, p_data: rawptr, components: c.int) -> bool --- // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
	...
	@(link_name="ImGui_ColorButton") ColorButton :: proc "c" (desc_id: cstring, col: Vec4, flags: ColorEditFlags) -> bool --- // Implied size = ImVec2(0, 0)
	@(link_name="ImGui_ColorButtonEx") ColorButtonEx :: proc "c" (desc_id: cstring, col: Vec4, flags: ColorEditFlags, size: Vec2) -> bool --- // display a color square/button, hover for details, return true when pressed.
	...
	@(link_name="ImGui_TreeNodeStr") TreeNodeStr :: proc "c" (str_id: cstring, fmt: cstring, #c_vararg args: ..any) -> bool --- // helper variation to easily decorelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
	...
	@(link_name="ImGui_CollapsingHeader") CollapsingHeader :: proc "c" (label: cstring, flags: TreeNodeFlags) -> bool --- // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
	@(link_name="ImGui_CollapsingHeaderBoolPtr") CollapsingHeaderBoolPtr :: proc "c" (label: cstring, p_visible: ^bool, flags: TreeNodeFlags) -> bool --- // when 'p_visible != NULL': if '*p_visible==true' display an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't display the header.
	@(link_name="ImGui_SetNextItemOpen") SetNextItemOpen :: proc "c" (is_open: bool, cond: Cond) --- // set next TreeNode/CollapsingHeader open state.
	// Widgets: Selectables
	// - A selectable highlights when hovered, and can display another color when selected.
	// - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
	@(link_name="ImGui_Selectable") Selectable :: proc "c" (label: cstring) -> bool --- // Implied selected = false, flags = 0, size = ImVec2(0, 0)
	@(link_name="ImGui_SelectableEx") SelectableEx :: proc "c" (label: cstring, selected: bool, flags: SelectableFlags, size: Vec2) -> bool --- // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
	@(link_name="ImGui_SelectableBoolPtr") SelectableBoolPtr :: proc "c" (label: cstring, p_selected: ^bool, flags: SelectableFlags) -> bool --- // Implied size = ImVec2(0, 0)
	@(link_name="ImGui_SelectableBoolPtrEx") SelectableBoolPtrEx :: proc "c" (label: cstring, p_selected: ^bool, flags: SelectableFlags, size: Vec2) -> bool --- // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
	// Widgets: List Boxes
	...
	@(link_name="ImGui_BeginMenuBar") BeginMenuBar :: proc "c" () -> bool --- // append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
	@(link_name="ImGui_EndMenuBar") EndMenuBar :: proc "c" () --- // only call EndMenuBar() if BeginMenuBar() returns true!
	@(link_name="ImGui_BeginMainMenuBar") BeginMainMenuBar :: proc "c" () -> bool --- // create and append to a full screen menu-bar.
	@(link_name="ImGui_EndMainMenuBar") EndMainMenuBar :: proc "c" () --- // only call EndMainMenuBar() if BeginMainMenuBar() returns true!
	@(link_name="ImGui_BeginMenu") BeginMenu :: proc "c" (label: cstring) -> bool --- // Implied enabled = true
	@(link_name="ImGui_BeginMenuEx") BeginMenuEx :: proc "c" (label: cstring, enabled: bool) -> bool --- // create a sub-menu entry. only call EndMenu() if this returns true!
	@(link_name="ImGui_EndMenu") EndMenu :: proc "c" () --- // only call EndMenu() if BeginMenu() returns true!
	@(link_name="ImGui_MenuItem") MenuItem :: proc "c" (label: cstring) -> bool --- // Implied shortcut = NULL, selected = false, enabled = true
	@(link_name="ImGui_MenuItemEx") MenuItemEx :: proc "c" (label: cstring, shortcut: cstring, selected: bool, enabled: bool) -> bool --- // return true when activated.
	@(link_name="ImGui_MenuItemBoolPtr") MenuItemBoolPtr :: proc "c" (label: cstring, shortcut: cstring, p_selected: ^bool, enabled: bool) -> bool --- // return true when activated + toggle (*p_selected) if p_selected != NULL
	// Tooltips
	...
	@(link_name="ImGui_BeginTooltip") BeginTooltip :: proc "c" () -> bool --- // begin/append a tooltip window.
	...
	@(link_name="ImGui_BeginItemTooltip") BeginItemTooltip :: proc "c" () -> bool --- // begin/append a tooltip window if preceding item was hovered.
	...
	@(link_name="ImGui_BeginPopup") BeginPopup :: proc "c" (str_id: cstring, flags: WindowFlags) -> bool --- // return true if the popup is open, and you can start outputting to it.
	@(link_name="ImGui_BeginPopupModal") BeginPopupModal :: proc "c" (name: cstring, p_open: ^bool, flags: WindowFlags) -> bool --- // return true if the modal is open, and you can start outputting to it.
	...
	@(link_name="ImGui_BeginPopupContextItem") BeginPopupContextItem :: proc "c" () -> bool --- // Implied str_id = NULL, popup_flags = 1
	@(link_name="ImGui_BeginPopupContextItemEx") BeginPopupContextItemEx :: proc "c" (str_id: cstring, popup_flags: PopupFlags) -> bool --- // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
	@(link_name="ImGui_BeginPopupContextWindow") BeginPopupContextWindow :: proc "c" () -> bool --- // Implied str_id = NULL, popup_flags = 1
	@(link_name="ImGui_BeginPopupContextWindowEx") BeginPopupContextWindowEx :: proc "c" (str_id: cstring, popup_flags: PopupFlags) -> bool --- // open+begin popup when clicked on current window.
	@(link_name="ImGui_BeginPopupContextVoid") BeginPopupContextVoid :: proc "c" () -> bool --- // Implied str_id = NULL, popup_flags = 1
	@(link_name="ImGui_BeginPopupContextVoidEx") BeginPopupContextVoidEx :: proc "c" (str_id: cstring, popup_flags: PopupFlags) -> bool --- // open+begin popup when clicked in void (where there are no windows).
	// Popups: query functions
	//  - IsPopupOpen(): return true if the popup is open at the current BeginPopup() level of the popup stack.
	//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup is open at the current BeginPopup() level of the popup stack.
	//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId + ImGuiPopupFlags_AnyPopupLevel: return true if any popup is open.
	@(link_name="ImGui_IsPopupOpen") IsPopupOpen :: proc "c" (str_id: cstring, flags: PopupFlags) -> bool --- // return true if the popup is open.
	// Tables
	...
	//        --------------------------------------------------------------------------------------------------------
	// - 5. Call EndTable()
	@(link_name="ImGui_BeginTable") BeginTable :: proc "c" (str_id: cstring, column: c.int, flags: TableFlags) -> bool --- // Implied outer_size = ImVec2(0.0f, 0.0f), inner_width = 0.0f
	...
	@(link_name="ImGui_TableNextColumn") TableNextColumn :: proc "c" () -> bool --- // append into the next column (or first column of next row if currently in last column). Return true when column is visible.
	@(link_name="ImGui_TableSetColumnIndex") TableSetColumnIndex :: proc "c" (column_n: c.int) -> bool --- // append into the specified column. Return true when column is visible.
	...
	@(link_name="ImGui_BeginTabBar") BeginTabBar :: proc "c" (str_id: cstring, flags: TabBarFlags) -> bool --- // create and append into a TabBar
	@(link_name="ImGui_EndTabBar") EndTabBar :: proc "c" () --- // only call EndTabBar() if BeginTabBar() returns true!
	@(link_name="ImGui_BeginTabItem") BeginTabItem :: proc "c" (label: cstring, p_open: ^bool, flags: TabItemFlags) -> bool --- // create a Tab. Returns true if the Tab is selected.
	@(link_name="ImGui_EndTabItem") EndTabItem :: proc "c" () --- // only call EndTabItem() if BeginTabItem() returns true!
	@(link_name="ImGui_TabItemButton") TabItemButton :: proc "c" (label: cstring, flags: TabItemFlags) -> bool --- // create a Tab behaving like a button. return true when clicked. cannot be selected in the tab bar.
	...
	@(link_name="ImGui_IsWindowDocked") IsWindowDocked :: proc "c" () -> bool --- // is current window docked into another window?
	// Logging/Capture
	...
	@(link_name="ImGui_BeginDragDropSource") BeginDragDropSource :: proc "c" (flags: DragDropFlags) -> bool --- // call after submitting an item which may be dragged. when this return true, you can call SetDragDropPayload() + EndDragDropSource()
	@(link_name="ImGui_SetDragDropPayload") SetDragDropPayload :: proc "c" (type: cstring, data: rawptr, sz: c.size_t, cond: Cond) -> bool --- // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
	...
	@(link_name="ImGui_BeginDragDropTarget") BeginDragDropTarget :: proc "c" () -> bool --- // call after submitting an item that may receive a payload. If this returns true, you can call AcceptDragDropPayload() + EndDragDropTarget()
	...
	@(link_name="ImGui_IsItemHovered") IsItemHovered :: proc "c" (flags: HoveredFlags) -> bool --- // is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.
	@(link_name="ImGui_IsItemActive") IsItemActive :: proc "c" () -> bool --- // is the last item active? (e.g. button being held, text field being edited. This will continuously return true while holding mouse button on an item. Items that don't interact will always return false)
	@(link_name="ImGui_IsItemFocused") IsItemFocused :: proc "c" () -> bool --- // is the last item focused for keyboard/gamepad navigation?
	@(link_name="ImGui_IsItemClicked") IsItemClicked :: proc "c" () -> bool --- // Implied mouse_button = 0
	@(link_name="ImGui_IsItemClickedEx") IsItemClickedEx :: proc "c" (mouse_button: MouseButton) -> bool --- // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
	@(link_name="ImGui_IsItemVisible") IsItemVisible :: proc "c" () -> bool --- // is the last item visible? (items may be out of sight because of clipping/scrolling)
	@(link_name="ImGui_IsItemEdited") IsItemEdited :: proc "c" () -> bool --- // did the last item modify its underlying value this frame? or was pressed? This is generally the same as the "bool" return value of many widgets.
	@(link_name="ImGui_IsItemActivated") IsItemActivated :: proc "c" () -> bool --- // was the last item just made active (item was previously inactive).
	@(link_name="ImGui_IsItemDeactivated") IsItemDeactivated :: proc "c" () -> bool --- // was the last item just made inactive (item was previously active). Useful for Undo/Redo patterns with widgets that require continuous editing.
	@(link_name="ImGui_IsItemDeactivatedAfterEdit") IsItemDeactivatedAfterEdit :: proc "c" () -> bool --- // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag moved). Useful for Undo/Redo patterns with widgets that require continuous editing. Note that you may get false positives (some widgets such as Combo()/ListBox()/Selectable() will return true even when clicking an already selected item).
	@(link_name="ImGui_IsItemToggledOpen") IsItemToggledOpen :: proc "c" () -> bool --- // was the last item open state toggled? set by TreeNode().
	@(link_name="ImGui_IsAnyItemHovered") IsAnyItemHovered :: proc "c" () -> bool --- // is any item hovered?
	@(link_name="ImGui_IsAnyItemActive") IsAnyItemActive :: proc "c" () -> bool --- // is any item active?
	@(link_name="ImGui_IsAnyItemFocused") IsAnyItemFocused :: proc "c" () -> bool --- // is any item focused?
	...
	@(link_name="ImGui_IsRectVisibleBySize") IsRectVisibleBySize :: proc "c" (size: Vec2) -> bool --- // test if rectangle (of given size, starting from cursor position) is visible / not clipped.
	@(link_name="ImGui_IsRectVisible") IsRectVisible :: proc "c" (rect_min: Vec2, rect_max: Vec2) -> bool --- // test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
	...
	@(link_name="ImGui_BeginChildFrame") BeginChildFrame :: proc "c" (id: ID, size: Vec2, flags: WindowFlags) -> bool --- // helper to create a child window / scrolling region that looks like a normal widget frame
	...
	@(link_name="ImGui_IsKeyDown") IsKeyDown :: proc "c" (key: Key) -> bool --- // is key being held.
	@(link_name="ImGui_IsKeyPressed") IsKeyPressed :: proc "c" (key: Key) -> bool --- // Implied repeat = true
	@(link_name="ImGui_IsKeyPressedEx") IsKeyPressedEx :: proc "c" (key: Key, repeat: bool) -> bool --- // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
	@(link_name="ImGui_IsKeyReleased") IsKeyReleased :: proc "c" (key: Key) -> bool --- // was key released (went from Down to !Down)?
	...
	@(link_name="ImGui_SetNextFrameWantCaptureKeyboard") SetNextFrameWantCaptureKeyboard :: proc "c" (want_capture_keyboard: bool) --- // Override io.WantCaptureKeyboard flag next frame (said flag is left for your application to handle, typically when true it instructs your app to ignore inputs). e.g. force capture keyboard when your widget is being hovered. This is equivalent to setting "io.WantCaptureKeyboard = want_capture_keyboard"; after the next NewFrame() call.
	// Inputs Utilities: Mouse specific
	// - To refer to a mouse button, you may use named enums in your code e.g. ImGuiMouseButton_Left, ImGuiMouseButton_Right.
	// - You can also use regular integer: it is forever guaranteed that 0=Left, 1=Right, 2=Middle.
	// - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking position (see 'lock_threshold' and 'io.MouseDraggingThreshold')
	@(link_name="ImGui_IsMouseDown") IsMouseDown :: proc "c" (button: MouseButton) -> bool --- // is mouse button held?
	@(link_name="ImGui_IsMouseClicked") IsMouseClicked :: proc "c" (button: MouseButton) -> bool --- // Implied repeat = false
	@(link_name="ImGui_IsMouseClickedEx") IsMouseClickedEx :: proc "c" (button: MouseButton, repeat: bool) -> bool --- // did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
	@(link_name="ImGui_IsMouseReleased") IsMouseReleased :: proc "c" (button: MouseButton) -> bool --- // did mouse button released? (went from Down to !Down)
	@(link_name="ImGui_IsMouseDoubleClicked") IsMouseDoubleClicked :: proc "c" (button: MouseButton) -> bool --- // did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note that a double-click will also report IsMouseClicked() == true)
	...
	@(link_name="ImGui_IsMouseHoveringRect") IsMouseHoveringRect :: proc "c" (r_min: Vec2, r_max: Vec2) -> bool --- // Implied clip = true
	@(link_name="ImGui_IsMouseHoveringRectEx") IsMouseHoveringRectEx :: proc "c" (r_min: Vec2, r_max: Vec2, clip: bool) -> bool --- // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
	@(link_name="ImGui_IsMousePosValid") IsMousePosValid :: proc "c" (mouse_pos: ^Vec2) -> bool --- // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
	@(link_name="ImGui_IsAnyMouseDown") IsAnyMouseDown :: proc "c" () -> bool --- // [WILL OBSOLETE] is any mouse button held? This was designed for backends, but prefer having backend maintain a mask of held mouse buttons, because upcoming input queue system will make this invalid.
	...
	@(link_name="ImGui_IsMouseDragging") IsMouseDragging :: proc "c" (button: MouseButton, lock_threshold: f32) -> bool --- // is mouse dragging? (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
	...
	@(link_name="ImGui_SetNextFrameWantCaptureMouse") SetNextFrameWantCaptureMouse :: proc "c" (want_capture_mouse: bool) --- // Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instucts your app to ignore inputs). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.
	// Clipboard Utilities
	...
	@(link_name="ImGui_DebugCheckVersionAndDataLayout") DebugCheckVersionAndDataLayout :: proc "c" (version_str: cstring, sz_io: c.size_t, sz_style: c.size_t, sz_vec2: c.size_t, sz_vec4: c.size_t, sz_drawvert: c.size_t, sz_drawidx: c.size_t) -> bool --- // This is called by IMGUI_CHECKVERSION() macro.
	// Memory Allocators
	...
	// Input Functions
	@(link_name="ImGuiIO_AddKeyEvent") IO_AddKeyEvent :: proc "c" (self: ^IO, key: Key, down: bool) --- // Queue a new key down/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
	@(link_name="ImGuiIO_AddKeyAnalogEvent") IO_AddKeyAnalogEvent :: proc "c" (self: ^IO, key: Key, down: bool, v: f32) --- // Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
	...
	@(link_name="ImGuiIO_AddMouseButtonEvent") IO_AddMouseButtonEvent :: proc "c" (self: ^IO, button: c.int, down: bool) --- // Queue a mouse button change
	...
	@(link_name="ImGuiIO_AddFocusEvent") IO_AddFocusEvent :: proc "c" (self: ^IO, focused: bool) --- // Queue a gain/loss of focus for the application (generally based on OS/platform focus of your window)
	...
	@(link_name="ImGuiIO_SetAppAcceptingEvents") IO_SetAppAcceptingEvents :: proc "c" (self: ^IO, accepting_events: bool) --- // Set master flag for accepting key/mouse/text events (default to true). Useful if you have native dialog boxes that are interrupting your application loop/refresh, and you want to disable events being queued while your app is frozen.
	...
	@(link_name="ImGuiTextFilter_Draw") TextFilter_Draw :: proc "c" (self: ^TextFilter, label: cstring, width: f32) -> bool --- // Helper calling InputText+Build
	...
	@(link_name="ImGuiListClipper_Step") ListClipper_Step :: proc "c" (self: ^ListClipper) -> bool --- // Call until it returns false. The DisplayStart/DisplayEnd fields will be set and you can process/draw those items.
	...
	@(link_name="ImFontGlyphRangesBuilder_GetBit") FontGlyphRangesBuilder_GetBit :: proc "c" (self: ^FontGlyphRangesBuilder, n: c.size_t) -> bool --- // Get bit n in the array
	...
	@(link_name="ImFontAtlas_Build") FontAtlas_Build :: proc "c" (self: ^FontAtlas) -> bool --- // Build pixels data. This is called automatically for you by the GetTexData*** functions.
	...
	@(link_name="ImFontAtlas_IsBuilt") FontAtlas_IsBuilt :: proc "c" (self: ^FontAtlas) -> bool --- // Bit ambiguous: used to detect when user didn't build texture but effectively we should check TexID != 0 except that would be backend dependent...
	...
	@(link_name="ImFont_AddRemapChar") Font_AddRemapChar :: proc "c" (self: ^Font, dst: Wchar, src: Wchar, overwrite_dst: bool) --- // Makes 'dst' character/glyph points to 'src' character/glyph. Currently needs to be called AFTER fonts have been built.
	...
	@(link_name="ImGui_ImageButtonImTextureID") ImageButtonImTextureID :: proc "c" (user_texture_id: TextureID, size: Vec2, uv0: Vec2, uv1: Vec2, frame_padding: c.int, bg_col: Vec4, tint_col: Vec4) -> bool --- // Use new ImageButton() signature (explicit item id, regular FramePadding)
	// OBSOLETED in 1.88 (from May 2022)
	@(link_name="ImGui_CaptureKeyboardFromApp") CaptureKeyboardFromApp :: proc "c" (want_capture_keyboard: bool) --- // Renamed as name was misleading + removed default value.
	@(link_name="ImGui_CaptureMouseFromApp") CaptureMouseFromApp :: proc "c" (want_capture_mouse: bool) --- // Renamed as name was misleading + removed default value.
