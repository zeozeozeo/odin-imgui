...
RenderDrawData :: proc(draw_data: ^imgui.DrawData, pass_encoder: wgpu.RenderPassEncoder) {
	...
	if fr.vertexBuffer == nil || len(fr.vertexBufferHost) < int(draw_data.TotalVtxCount) {
		...
		fr.vertexBuffer = wgpu.DeviceCreateBuffer(
			bd.device,
			&{
	...
	if fr.indexBuffer == nil || len(fr.indexBufferHost) < int(draw_data.TotalIdxCount) {
		...
		fr.indexBuffer = wgpu.DeviceCreateBuffer(
			bd.device,
			&{
				label = "Dear ImGui Index buffer",
				usage = {.CopyDst, .Index},
				size = cast(u64)mem.align_forward_uint(
					len(fr.indexBufferHost) * size_of(imgui.DrawIdx),
					4,
				),
			},
		)
	...
	for n in 0 ..< draw_data.CmdListsCount {
		...
		vtx_dst_off += copy(
			vtx_dst[vtx_dst_off:],
			(cast([^]imgui.DrawVert)draw_list.VtxBuffer.Data)[:draw_list.VtxBuffer.Size],
		)
		idx_dst_off += copy(
			idx_dst[idx_dst_off:],
			(cast([^]imgui.DrawIdx)draw_list.IdxBuffer.Data)[:draw_list.IdxBuffer.Size],
		)
	}
	...
	for n in 0 ..< draw_data.CmdListsCount {
		...
		for cmd_i in 0 ..< draw_list.CmdBuffer.Size {
			...
			if pcmd.UserCallback != nil {
				...
				if is_new {
					bind_group := CreateImageBindGroup(
						bd.renderResources.imageBindGroupLayout,
						transmute(wgpu.TextureView)tex_id,
					)
					val^ = bind_group
				}
...
SetupRenderState :: proc(
	...
	wgpu.QueueWriteBuffer(
		...
		cast(u64)offset_of(Uniforms, MVP),
		&mvp,
		size_of(mvp),
	)
	...
	wgpu.QueueWriteBuffer(
		bd.defaultQueue,
		bd.renderResources.uniforms,
		cast(u64)offset_of(Uniforms, gamma),
		&gamma,
		size_of(gamma),
	)
	...
	wgpu.RenderPassEncoderSetViewport(
		...
		draw_data.FramebufferScale.y * draw_data.DisplaySize.y,
		0,
		1,
	)
	...
	wgpu.RenderPassEncoderSetVertexBuffer(
		...
		fr.vertexBuffer,
		0,
		u64(len(fr.vertexBufferHost)) * size_of(imgui.DrawVert),
	)
	wgpu.RenderPassEncoderSetIndexBuffer(
		...
		0,
		u64(len(fr.indexBufferHost)) * size_of(imgui.DrawIdx),
	)
...
CreateFontsTexture :: proc() {
	...
	bd.renderResources.fontTexture = wgpu.DeviceCreateTexture(
		bd.device,
		&{
			label = "Dear ImGui font Texture",
			dimension = ._2D,
			size = {width = cast(u32)width, height = cast(u32)height, depthOrArrayLayers = 1},
			sampleCount = 1,
			format = .RGBA8Unorm,
			mipLevelCount = 1,
	...
	bd.renderResources.fontTextureView = wgpu.TextureCreateView(
	...
	wgpu.QueueWriteTexture(
		bd.defaultQueue,
		&{texture = bd.renderResources.fontTexture, aspect = .All},
		pixels,
		cast(uint)width * cast(uint)height * cast(uint)size_pp,
		&{bytesPerRow = cast(u32)width * cast(u32)size_pp, rowsPerImage = cast(u32)height},
		&{cast(u32)width, cast(u32)height, 1},
	)

	bd.renderResources.sampler = wgpu.DeviceCreateSampler(
		bd.device,
		&{
			minFilter = .Linear,
			magFilter = .Linear,
	...
	imgui.FontAtlas_SetTexID(io.Fonts, transmute(u64)bd.renderResources.fontTextureView)
}
...
CreateUniformBuffer :: proc() {
	...
	bd.renderResources.uniforms = wgpu.DeviceCreateBuffer(
		bd.device,
		&{
...
CreateDeviceObjects :: proc() {
	...
	bg_layouts[0] = wgpu.DeviceCreateBindGroupLayout(
		bd.device,
		&{
			entryCount = 2,
			entries = raw_data(
				[]wgpu.BindGroupLayoutEntry {
					{visibility = {.Vertex, .Fragment}, buffer = {type = .Uniform}},
					{binding = 1, visibility = {.Fragment}, sampler = {type = .Filtering}},
				},
			),
		},
	)
	bg_layouts[1] = wgpu.DeviceCreateBindGroupLayout(
		bd.device,
		&{
			entryCount = 1,
	...
	bd.pipelineState = wgpu.DeviceCreateRenderPipeline(
		...
		&{
			...
			depthStencil = nil if bd.depthStencilFormat == .Undefined else &{format = bd.depthStencilFormat, depthWriteEnabled = .False, depthCompare = .Always, stencilFront = {.Always, .Keep, .Keep, .Keep}, stencilBack = {.Always, .Keep, .Keep, .Keep}},
		},
	)
...
InvalidateDeviceObjects :: proc() {
	...
	io := imgui.GetIO()
	imgui.FontAtlas_SetTexID(io.Fonts, 0)
